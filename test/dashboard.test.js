...
test("dashboard exposes operational budgets and deliberate deeper research", async () => {
  ...
  for (const text of ["Research status & budget", "Technical operations telemetry", "estimated_cost_usd", "web_search_calls", "bounded_sources", "selected_provider", "local allowances", "Fast foundation", "duplicate requests avoided", "Deep targeted unresolved components first", "Operational budgets do not certify evidence completeness"]) assert.match(source, new RegExp(text));
});
