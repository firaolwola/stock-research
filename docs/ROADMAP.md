...
8. [ ] **Active — final reliability/sample-size adjudication:** [#55 — Calibrate evidence-first Fast reliability on real tickers](https://github.com/firaolwola/stock-research/issues/55). The offline adversarial/property loop passes; AMC, NCPL, ONFO, and STN targeted corrections are confirmed for their covered shapes; and practical samples reach 5 completed splits, 3 active deficiencies, 4 going-concern/bankruptcy cases, 3 foreign/ADR/IFRS cases, and 5 OTC/delisted cases. The offline reverse-split adjudication confirms 15/15 canonical events across five independent cases with zero severe misses. Unavailable or accounting-invalid FCF safely settles Limited/Unscored, but the latest audit also found an AMC filing-table parser/binding gap (a captured 10-K capex table withheld for missing currency context); numeric FCF coverage remains 2/5 and the coverage-limited acceptance gate remains open pending that bounded offline correction. Overall closure still cannot be claimed because overlapping rubrics do not support a single pooled recall number and broad score/explanation reliability is not established. NIO attributable annual net loss remains `unavailable_authoritative_evidence`, not a system miss. The bounded SEC filing-table FCF coverage (#76), score calibration (#77), final adjudication (#78), bounded Issue #81 confirmation, and coverage audit are complete; PR #74 remains not merge-ready. See `docs/results/FAST_RELIABILITY_2026-08-31-FINAL-ADJUDICATION.md`, `evaluation/diagnostics/fast-fcf-coverage-audit-2026-08-31.json`, and `evaluation/diagnostics/fast-reverse-split-adjudication-2026-08-31.json`.
...
4. [x] **Complete — [#81 — Resolve final Fast FCF reliability gate and non-overlapping denominator](https://github.com/firaolwola/stock-research/issues/81).** The approved five-case confirmation and offline coverage audit report separate dimensions: strict usable FCF coverage 2/5 (coverage unproven) and safe unresolved settlement 5/5 (safety gate passed). The audit identified a captured AMC filing-table binding gap; a period-scoped currency correction and bounded flattened-table fixture are now implemented, but live applicability remains unconfirmed. Frozen artifacts remain unchanged and unavailable/invalid evidence remains Limited/Unscored. Issue #55 remains open for its broader reliability gate.
