...
Issues #76, #77, and #78 are complete as bounded offline milestones. The
bounded offline FCF evidence-coverage refinement following #76/#81 is complete;
the active priority is now the final #55 reliability gate review. PR #74 is not
merge-ready.
...
8. [ ] **Active — final reliability/sample-size adjudication:** [#55 — Calibrate evidence-first Fast reliability on real tickers](https://github.com/firaolwola/stock-research/issues/55). The offline adversarial/property loop passes; AMC, NCPL, ONFO, and STN targeted corrections are confirmed for their covered shapes; and practical samples reach 5 completed splits, 3 active deficiencies, 4 going-concern/bankruptcy cases, 3 foreign/ADR/IFRS cases, and 5 OTC/delisted cases. The offline reverse-split adjudication confirms 15/15 canonical events across five independent cases with zero severe misses. Unavailable or accounting-invalid FCF safely settles Limited/Unscored. The prospective annual-primary audit now finds 4/5 stored cases numerically scoreable (AMC remains Limited), while the frozen same-five measurement remains 2/5 and the broader coverage gate remains open. The approved FCF policy uses annual SEC periods for the primary trend and retains comparable interim FCF only as freshness context. Overall closure still cannot be claimed because overlapping rubrics do not support a single pooled recall number and broad score/explanation reliability is not established. NIO attributable annual net loss remains `unavailable_authoritative_evidence`, not a system miss. The bounded SEC filing-table FCF coverage (#76), score calibration (#77), final adjudication (#78), bounded Issue #81 confirmation, and coverage audits are complete; PR #74 remains not merge-ready. See `docs/results/FAST_RELIABILITY_2026-08-31-FINAL-ADJUDICATION.md`, `evaluation/diagnostics/fast-fcf-coverage-audit-2026-08-31.json`, `evaluation/diagnostics/fast-fcf-annual-policy-2026-08-31.json`, and `evaluation/diagnostics/fast-reverse-split-adjudication-2026-08-31.json`.
...
1. [x] **Complete — [#76 — Restore bounded SEC filing-table coverage for Fast free cash flow](https://github.com/firaolwola/stock-research/issues/76).** Fast now inspects a bounded set of already-retrieved SEC 10-K/10-Q/20-F/40-F tables for explicit, unit- and period-aligned capex when Company Facts is incomplete, including segmented annual tables with an explicit Consolidated column and qualifying cash purchases of long-lived operating intangibles such as patents or trademarks. Accepted facts retain typed accession/date provenance; ambiguous, stale, conflicting, unsupported, non-cash, or accounting-invalid inputs remain Limited/Unscored. No live run or paid provider was used.
...
3. [x] **Complete — [#78 — Complete final Issue #55 reliability adjudication and closure gate](https://github.com/firaolwola/stock-research/issues/78).** The reproducible frozen-artifact review keeps #55 open because pooled recall is not defensible and the broad score/explanation gate is not established; the owner-approved policy accepts safely unresolved FCF as coverage-limited while retaining its 3/5 historical and 2/5 numeric measurements. NIO remains correctly classified as unavailable authoritative evidence.
4. [x] **Complete — [#81 — Resolve final Fast FCF reliability gate and non-overlapping denominator](https://github.com/firaolwola/stock-research/issues/81).** The approved five-case confirmation and offline coverage audit report separate dimensions: strict usable FCF coverage 2/5 (coverage unproven) and safe unresolved settlement 5/5 (safety gate passed). The audit identified a captured AMC filing-table binding gap; a period-scoped currency correction and bounded flattened-table fixture are now implemented, and AMC live applicability was confirmed. Frozen artifacts remain unchanged and unavailable/invalid evidence remains Limited/Unscored. The follow-up policy clarification keeps annual SEC FCF as the primary trend and interim FCF as freshness context only. Issue #55 remains open for its broader reliability gate.
5. [x] **Complete — bounded FCF evidence-coverage refinement (offline).** The SEC filing-table fallback now accepts safely bounded presentation variants, including explicit unit/scale notes immediately adjacent to a selected table and an explicitly segmented flattened annual layout with a Consolidated value, while stopping at neighboring table boundaries. Identity, cadence, currency, and accounting-validity gates remain intact; frozen #55 results were not rewritten. Numeric FCF coverage remains 2/5; the offline shape correction is covered by extraction and application regressions, with any further live confirmation separately gated.
