...
8. [ ] **Active — correct live MULN competing-ratio canonicalization offline:** [#55 — Calibrate evidence-first Fast reliability on real tickers](https://github.com/firaolwola/stock-research/issues/55). Batch 3 reached 83/86 material checks (96.51%). The recursive-plan MULN run retrieved all five frozen completed splits but still exposed a false same-date 1-for-100 event and a written-ratio truncation that became false 1-for-2 evidence. Both were accepted despite competing-ratio diagnostics. NIO remains correctly Limited under unavailable authoritative evidence. No additional live run or expansion is authorized.
