...
8. [ ] **Active — final MULN verification blocked by nested-plan runner failure:** [#55 — Calibrate evidence-first Fast reliability on real tickers](https://github.com/firaolwola/stock-research/issues/55). Batch 3 reached 83/86 material checks (96.51%). Sparse-5 remained at 14/16 (87.5%). Stored-live MULN regressions pass, but the approved final process stopped during local plan validation because the runner did not resolve the grandparent baseline reference. It made no network or paid call and was not retried. NIO remains correctly Limited under unavailable authoritative evidence. No additional live run or expansion is authorized.
