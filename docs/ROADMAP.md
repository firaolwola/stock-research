...
8. [ ] **Active — await owner decision on a fresh MULN-only verification:** [#55 — Calibrate evidence-first Fast reliability on real tickers](https://github.com/firaolwola/stock-research/issues/55). Batch 3 reached 83/86 material checks (96.51%). Sparse-5 remained at 14/16 (87.5%). Stored-live MULN regressions pass, and recursive plan resolution now fixes the Verification-3 runner failure offline with frozen ancestry, provenance, and pre-network validation. The failed authorization is not reusable. NIO remains correctly Limited under unavailable authoritative evidence. No additional live run or expansion is authorized.
