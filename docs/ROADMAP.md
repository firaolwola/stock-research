...
4. [x] **Complete — [#81 — Resolve final Fast FCF reliability gate and non-overlapping denominator](https://github.com/firaolwola/stock-research/issues/81).** The approved five-case confirmation and offline coverage audit report separate dimensions: strict usable FCF coverage 2/5 (coverage unproven) and safe unresolved settlement 5/5 (safety gate passed). The refreshed audit identifies one captured AMC filing-table candidate withheld for missing currency context, so the coverage-limited acceptance gate remains open pending offline correction; frozen artifacts remain unchanged and unavailable/invalid evidence remains Limited/Unscored. Issue #55 remains open for its broader reliability gate.
5. [x] **Complete — bounded FCF evidence-coverage refinement (offline).** The SEC filing-table fallback now accepts safely bounded presentation variants, including explicit unit/scale notes immediately adjacent to a selected table, while stopping at neighboring table boundaries. Identity, cadence, currency, and accounting-validity gates remain intact; frozen #55 results were not rewritten. The approved AMC confirmation validated period-scoped currency binding but still found a 10-K period/value column mismatch, so numeric FCF coverage remains 2/5 and the next step is offline table-shape correction before any further live run.
