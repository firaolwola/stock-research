...
for (const [name, expected] of [["run-summary.json", plan.preserve_failed_verification_3.run_summary_sha256], ["raw/MULN.json", plan.preserve_failed_verification_3.raw_muln_sha256]]) {
  const bytes = await readFile(path.join(root, ...plan.preserve_failed_verification_3.directory.split("/"), ...name.split("/")));
  if (createHash("sha256").update(bytes).digest("hex") !== expected) throw new Error(`Refusing to run because the Verification-3 artifact ${name} changed.`);
}
...
for (const [name, expected] of [["run-summary.json", plan.preserve_verification_6.run_summary_sha256], ["raw/MULN.json", plan.preserve_verification_6.raw_muln_sha256], ["summary.json", plan.preserve_verification_6.summary_sha256], ["review/MULN.json", plan.preserve_verification_6.review_muln_sha256]]) {
  const bytes = await readFile(path.join(root, ...plan.preserve_verification_6.directory.split("/"), ...name.split("/")));
  if (createHash("sha256").update(bytes).digest("hex") !== expected) throw new Error(`Refusing to run because the Verification-6 artifact ${name} changed.`);
}
