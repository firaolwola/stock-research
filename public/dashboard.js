...
function renderOperations(operations) {
  ...
  if (operations.stage === "fast" && operations.bounded_sources) {
    const bounded = operations.bounded_sources; const marketProvider = bounded.providers?.market?.selected_provider; const newsProvider = bounded.providers?.news?.selected_provider;
    const quotaText = (bounded.providers?.quotas ?? []).filter((item) => item.configured).map((item) => `${formatLabel(item.provider)} ${item.daily_used}/${item.daily_limit}`).join(" · ");
    body.append(element("p", "muted", `Bounded sources: ${formatLabel(bounded.status)} · Nasdaq ${formatLabel(bounded.nasdaq)} · news ${formatLabel(bounded.news)}${newsProvider ? ` via ${formatLabel(newsProvider)}` : ""}${bounded.news_reason ? ` (${formatLabel(bounded.news_reason)})` : ""} · market ${formatLabel(bounded.market)}${marketProvider ? ` via ${formatLabel(marketProvider)}` : ""}${bounded.market_reason ? ` (${formatLabel(bounded.market_reason)})` : ""} · ${bounded.request_count} network requests${quotaText ? ` · local allowances ${quotaText}` : ""}`));
  }
