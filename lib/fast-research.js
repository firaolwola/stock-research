...
export function createFastDomainSchema(schema, domain) {
  ...
  if (domain === "financial") {
    delete projected.$defs.financialMetric.properties.observations;
    delete projected.$defs.financialMetric.properties.annual_observations;
    delete projected.$defs.financialAssessment.properties.shares_outstanding;
  }
...
const unknownMetric = (label) => ({ state: "unknown", label, value: null, unit: null, period_start: null, period_end: null, trend: "unknown", comparison_period_start: null, comparison_period_end: null, observations: [], annual_observations: [], summary: `${label} was not established by completed Fast evidence.`, claim_ids: [] });
const unknownFinancial = (settled = false) => ({
  state: "unknown", as_of: null, reporting_currency: null, summary: settled ? "Immediate financial-risk evidence is Limited." : "Immediate financial-risk research is pending.", coverage_notes: ["The financial Fast domain did not complete."],
  metrics: Object.fromEntries(["cash", "cash_burn", "revenue", "profitability", "operating_cash_flow", "free_cash_flow", "debt"].map((key) => [key, unknownMetric(key.replaceAll("_", " "))])),
  shares_outstanding: { state: "unknown", summary: settled ? "Reported shares outstanding are Limited." : "Reported shares outstanding are pending.", observations: [], claim_ids: [] },
  going_concern: { state: "unknown", as_of: null, summary: settled ? "Going-concern evidence is Limited." : "Going-concern evidence is pending.", claim_ids: [] }, material_warnings: []
});
const unknownFactor = (label, settled = false) => ({ rating: "unknown", explanation: settled ? `${label} is Limited because catalyst evidence did not complete.` : `${label} is pending completed catalyst evidence.`, claim_ids: [] });
const unknownCatalyst = (settled = false) => ({
  current: { state: "unknown", classification: "unknown", title: settled ? "Current catalyst Limited" : "Current catalyst pending", event_date: null, summary: "The catalyst Fast domain did not complete.", confidence: "unknown", claim_ids: [], factors: Object.fromEntries(["recency", "specificity", "credibility", "novelty", "potential_significance"].map((key) => [key, unknownFactor(key.replaceAll("_", " "), settled)])) },
  ...
  favorable_evidence_claim_ids: [], unfavorable_evidence_claim_ids: [], uncertainty: [settled ? "Current catalyst evidence is Limited." : "Current catalyst evidence is pending."],
...
function catalystSection(assessment, settled = false) {
  const current = assessment?.current;
  if (!current) return unknownSection(settled ? "Current catalyst evidence is Limited." : "Current catalyst research is pending.");
  return {
    state: current.state,
...
function financialSection(assessment, settled = false) {
  if (!assessment) return unknownSection(settled ? "Immediate financial-risk context is Limited." : "Immediate financial-risk context is pending.");
...
export function assembleFastReport(ticker, domainResults, { generatedAt = new Date().toISOString(), final = false, terminalReason = null } = {}) {
  ...
  for (const domain of Object.keys(FAST_DOMAINS)) {
    if (!byDomain[domain]) limitations.push({ code: terminalReason ?? `fast-${domain}-${final ? "limited" : identityAgrees ? "pending" : "identity-mismatch"}`, explanation: terminalReason === "identity_unresolved" ? "The requested security identity could not be resolved safely from the current map or bounded authoritative historical aliases." : final ? `The ${domain} Fast domain did not complete and is Limited.` : identityAgrees ? `The ${domain} Fast domain did not complete and remains pending.` : "Domain evidence was not merged because issuer/security identity did not agree.", affected_sections: domain === "capital" ? ["issuer", "reverse_splits", "dilution"] : domain === "catalyst" ? ["compliance_and_warnings", "catalysts_and_news", "catalyst_assessment"] : ["dividends", "financial_context", "financial_assessment"] });
  }
  ...
  return {
    ...
    metadata: { as_of: generatedAt, generated_at: generatedAt, stage: "fast", completion_status: usable.length === 3 || final ? "partial" : "pending", research_windows: [{ topic: "fast_material_risk", start: `${Number(day.slice(0, 4)) - 5}${day.slice(4)}`, end: day }], coverage_limitations: limitations },
    security: capital?.security ?? { ticker, name: final ? `${ticker} security identity unresolved` : `${ticker} security identity pending`, security_type: "unknown", listing_venue: "Unknown", listing_status: "unknown", evidence_state: final ? "limited_coverage" : "unknown", claim_ids: [] },
    issuer: capital?.issuer ?? { legal_name: final ? "Issuer identity unresolved" : identity?.issuer_legal_name ?? "Issuer identity pending", cik: identity?.cik ?? null, identity_state: final ? "limited_coverage" : "unknown", identity_confidence: "unknown", prior_identities: [], claim_ids: [] },
    sections: {
      reverse_splits: capital?.reverse_splits ?? unknownSection(final ? "Reverse-split evidence is Limited." : "Reverse-split research is pending."),
      dilution: capital?.dilution ?? unknownSection(final ? "Dilution and offering evidence is Limited." : "Dilution and offering research is pending."),
      dividends: financial?.dividends ?? unknownSection(final ? "Dividend applicability and status are Limited." : "Dividend applicability and status are pending."),
      compliance_and_warnings: catalyst?.compliance_and_warnings ?? unknownSection(final ? "Compliance and accounting-warning evidence is Limited." : "Compliance and accounting-warning research is pending."),
      financial_context: financialSection(financial?.financial_assessment, final),
      catalysts_and_news: catalystSection(catalyst?.catalyst_assessment, final)
    },
    financial_assessment: financial?.financial_assessment ?? unknownFinancial(final),
    catalyst_assessment: catalyst ? { ...catalyst.catalyst_assessment, historical_analogues: unknownCatalyst(final).historical_analogues } : unknownCatalyst(final),
    claims,
    sources
  };
}
