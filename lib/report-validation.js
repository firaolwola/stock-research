...
export function validateFinalReportSettlement(report) {
  const errors = [];
  if (report?.metadata?.completion_status === "pending") errors.push({ type: "semantic", path: "/metadata/completion_status", keyword: "terminalSettlement", message: "a final report cannot remain pending" });
  for (const limitation of report?.metadata?.coverage_limitations ?? []) {
    if (/-pending$/.test(limitation.code ?? "") || /remains pending|research is pending/i.test(limitation.explanation ?? "")) errors.push({ type: "semantic", path: "/metadata/coverage_limitations", keyword: "terminalSettlement", message: "a final report cannot declare in-progress coverage" });
  }
...
export function createReportValidator(schema) {
  ...
  return function validateReport(report) {
    ...
    const referencedClaimIds = [
      ...
      ...(report.financial_assessment.shares_outstanding?.claim_ids ?? []),
    ...
    for (const [scoreName, score] of Object.entries(report.scores)) {
      ...
      if (!["2.0.0", "2.1.0"].includes(score.methodology_version)) addError(`score ${scoreName} must use a supported scoring methodology`);
      ...
      if (score.state === "confirmed") {
        ...
        if (scoreName === "financial_health" || scoreName.startsWith("financial_") && scoreName.endsWith("_trend")) {
          const nonSecClaim = score.claim_ids.find((claimId) => !(claims.get(claimId)?.source_ids ?? []).some((sourceId) => {
            const source = sources.get(sourceId);
            if (source?.source_type === "sec_filing") return true;
            try { return /(^|\.)sec\.gov$/i.test(new URL(source?.url).hostname); } catch { return false; }
          }));
          if (nonSecClaim) addError(`financial score ${scoreName} must use SEC-authoritative evidence only`);
        }
      }
    ...
    for (const [metricName, metric] of Object.entries(financial.metrics)) {
      ...
      if (metric.interim_context) {
        validateEvidenceReferences(`financial metric ${metricName} interim context`, metric.interim_context.state, metric.interim_context.claim_ids);
        if (metric.interim_context.latest_period_start > metric.interim_context.latest_period_end || metric.interim_context.comparison_period_start > metric.interim_context.comparison_period_end) addError(`financial metric ${metricName} interim context period start cannot be after end`);
        if (metric.interim_context.unit !== metric.unit) addError(`financial metric ${metricName} interim context must use the metric unit`);
        if (metric.interim_context.latest_period_end <= metric.interim_context.comparison_period_end) addError(`financial metric ${metricName} interim context must be chronological`);
      }
      const observations = metric.observations ?? [];
      const annualObservations = metric.annual_observations ?? [];
      ...
      let previousAnnual = null;
      for (const [index, observation] of annualObservations.entries()) {
        validateEvidenceReferences(`financial metric ${metricName} annual observation ${index + 1}`, "confirmed", observation.claim_ids);
        if (observation.period_start > observation.period_end) addError(`financial metric ${metricName} annual observation period start cannot be after end`);
        if (financial.as_of && observation.period_end > financial.as_of) addError(`financial metric ${metricName} annual observation cannot end after the assessment as-of date`);
        if (metric.unit && observation.unit !== metric.unit) addError(`financial metric ${metricName} annual observations must use the metric unit`);
        if (previousAnnual && observation.period_end <= previousAnnual.period_end) addError(`financial metric ${metricName} annual observations must be unique and chronological`);
        const duration = Math.round((new Date(`${observation.period_end}T00:00:00Z`) - new Date(`${observation.period_start}T00:00:00Z`)) / 86_400_000);
        if (duration !== 0 && (duration < 330 || duration > 400)) addError(`financial metric ${metricName} annual observations must use annual or point-in-time periods`);
        previousAnnual = observation;
      }
      ...
      if (metric.state !== "confirmed" && annualObservations.length > 0) addError(`unresolved financial metric ${metricName} cannot contain annual chart observations`);
    ...
    const shareSeries = financial.shares_outstanding;
    if (shareSeries) {
      validateEvidenceReferences("shares outstanding", shareSeries.state, shareSeries.claim_ids);
      let previous = null;
      for (const [index, observation] of shareSeries.observations.entries()) {
        validateEvidenceReferences(`shares outstanding observation ${index + 1}`, "confirmed", observation.claim_ids);
        if (observation.unit !== "shares") addError("shares outstanding observations must use shares, not float or potential dilution units");
        if (observation.period_start !== observation.period_end) addError("shares outstanding observations must be reported point-in-time values");
        if (financial.as_of && observation.period_end > financial.as_of) addError("shares outstanding observation cannot end after the assessment as-of date");
        if (previous && observation.period_end <= previous.period_end) addError("shares outstanding observations must be unique and chronological");
        previous = observation;
      }
      const annualShares = shareSeries.annual_observations ?? [];
      let previousAnnualShare = null;
      for (const [index, observation] of annualShares.entries()) {
        validateEvidenceReferences(`annual shares outstanding observation ${index + 1}`, "confirmed", observation.claim_ids);
        if (observation.unit !== "shares" || observation.period_start !== observation.period_end) addError("annual shares outstanding observations must use point-in-time shares");
        if (previousAnnualShare && observation.period_end <= previousAnnualShare.period_end) addError("annual shares outstanding observations must be unique and chronological");
        previousAnnualShare = observation;
      }
      if (shareSeries.state === "confirmed" && shareSeries.observations.length === 0) addError("confirmed shares outstanding requires at least one reported observation");
      if (shareSeries.state !== "confirmed" && shareSeries.observations.length > 0) addError("unresolved shares outstanding cannot contain chart observations");
      if (shareSeries.state !== "confirmed" && annualShares.length > 0) addError("unresolved shares outstanding cannot contain annual chart observations");
    }
