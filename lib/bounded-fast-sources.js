...
function mergeReport(base, additions) {
  ...
  if (additions.marketItems.length) {
    const section = report.sections.financial_context;
    section.items.push(...additions.marketItems);
    section.claim_ids = [...new Set([...section.claim_ids, ...additions.marketItems.flatMap((item) => item.claim_ids)])];
  }
...
export function createBoundedFastSourceClient({ fetchImpl = globalThis.fetch, alphaVantageApiKey = "", now = () => Date.now(), logger = console, sourceTimeoutMs = SOURCE_TIMEOUT_MS, alphaRequestGapMs = ALPHA_REQUEST_GAP_MS } = {}) {
  ...
  return { async enrich(ticker, deterministic, { budget, onProgress } = {}) {
    ...
    const series = marketResult?.data?.["Time Series (Daily)"] ?? {}; const dates = Object.keys(series).filter((value) => /^\d{4}-\d{2}-\d{2}$/.test(value)).sort(); const latestDate = dates.at(-1); const bar = latestDate ? series[latestDate] : null;
    ...
    if (!marketReason) {
      ...
      const previous = series[dates.at(-2)] ? Number(series[dates.at(-2)]["4. close"]) : null;
      const baselineVolumes = dates.slice(-21, -1).map((day) => Number(series[day]?.["5. volume"])).filter((value) => Number.isFinite(value) && value >= 0);
      const averageVolume = baselineVolumes.length >= 5 ? baselineVolumes.reduce((sum, value) => sum + value, 0) / baselineVolumes.length : null;
      if (Number.isFinite(previous) && previous > 0 && Number.isFinite(averageVolume) && averageVolume > 0) {
        additions.marketItems.push(
          { id: "alpha-eod-price-change", kind: "other", title: "End-of-day price change", state: "confirmed", summary: "Latest close compared with the immediately preceding daily close.", event_date: latestDate, value: (Number(bar["4. close"]) / previous - 1) * 100, unit: "price_change_percent", claim_ids: [claimId] },
          { id: "alpha-eod-relative-volume", kind: "other", title: "End-of-day relative volume", state: "confirmed", summary: `Latest volume compared with a ${baselineVolumes.length}-session bounded baseline.`, event_date: latestDate, value: Number(bar["5. volume"]) / averageVolume, unit: "volume_ratio", claim_ids: [claimId] }
        );
      } else additions.limitations.push({ code: "alpha-market-insufficient-baseline", explanation: "End-of-day observation was retrieved, but there was not enough valid history for price-change and relative-volume scoring context.", affected_sections: ["catalyst_assessment"] });
