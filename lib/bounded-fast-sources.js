...
export function createBoundedFastSourceClient({ fetchImpl = globalThis.fetch, alphaVantageApiKey = "", now = () => Date.now(), logger = console, sourceTimeoutMs = SOURCE_TIMEOUT_MS } = {}) {
  ...
  const alpha = async (key, params, context) => {
    ...
    if (result?.data?.Note || result?.data?.Information) { context.budget?.recordSource(context.source, "limited", { reason: "provider_quota" }); return { unavailable: "provider_quota" }; }
    return result;
  };
  return { async enrich(ticker, deterministic, { budget, onProgress } = {}) {
    ...
    const [directories, newsResult, marketResult] = await Promise.all([
      Promise.all([
      ...
      alpha(`alpha:news:${ticker}`, { function: "NEWS_SENTIMENT", tickers: ticker, sort: "LATEST", limit: "10" }, { budget, counter, source: "alpha_vantage:news" }),
      alpha(`alpha:market:${ticker}`, { function: "TIME_SERIES_DAILY", symbol: ticker, outputsize: "compact" }, { budget, counter, source: "alpha_vantage:market" })
    ]);
    ...
    const series = marketResult?.data?.["Time Series (Daily)"] ?? {}; const latestDate = Object.keys(series).sort().at(-1); const bar = latestDate ? series[latestDate] : null;
    if (bar && Number.isFinite(Number(bar["4. close"])) && Number.isFinite(Number(bar["5. volume"]))) {
    ...
    } else additions.limitations.push({ code: `alpha-market-${marketResult?.unavailable ?? "no-data"}`, explanation: "End-of-day price and volume context is Limited and cannot affect risk scoring.", affected_sections: ["catalyst_assessment"] });
