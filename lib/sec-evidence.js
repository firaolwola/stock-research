...
function companyFactsFlowCurrency(companyFacts) {
  const concepts = [
    companyFacts?.facts?.["us-gaap"]?.NetCashProvidedByUsedInOperatingActivities,
    companyFacts?.facts?.["ifrs-full"]?.CashFlowsFromUsedInOperatingActivities
  ];
  const units = [...new Set(concepts.flatMap((concept) => Object.keys(concept?.units ?? {})).filter((unit) => /^[A-Z]{3}$/.test(unit)))];
