...
function applyExtractedFindings(fragments, findings, retrievedAt, records, corporateActionDiagnostics = []) {
  ...
  const reverse = reverseCandidates.filter((item) => {
    const diagnostic = corporateActionDiagnostics.find((candidate) => candidate.source_accession === item.accession && candidate.local_text_span_id === item.local_text_span_id);
