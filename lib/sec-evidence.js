...
function financialFragment(identity, companyFacts, retrievedAt, records, { filingCapexFacts = [], filingCapexDiagnostics = [] } = {}) {
  ...
  const addFact = (key, fact, { label = fact?.label, value = fact?.val, state = "confirmed", summary, comparable = [fact], annual = [] } = {}) => {
    ...
    const sourceFacts = (item) => item?.source_facts?.length ? item.source_facts : [item];
  ...
  const capexTags = [["us-gaap", "PaymentsToAcquirePropertyPlantAndEquipment", "Capital expenditures"], ["ifrs-full", "PurchaseOfPropertyPlantAndEquipment", "Capital expenditures"], ["sec-filing", "CapitalExpenditures", "Capital expenditures"]];
  ...
  const fcfPairs = alignedFactPairs(factCandidates(companyFacts, ocfTags), factCandidates(companyFacts, capexTags));
  ...
  if (fcfPair) {
    ...
    const comparableFcf = alignDerivedPeriods(factCandidates(companyFacts, ocfTags), factCandidates(companyFacts, capexTags), (left, right) => ({ ...left, val: left.val - Math.abs(right.val), label: "Free cash flow", source_facts: [left, right] }));
    const summary = `Free cash flow of ${fcf.val} ${fcf.unit} is calculated as operating cash flow ${alignedOcf.val} less capital expenditures ${Math.abs(alignedCapex.val)} for ${fcf.start || fcf.end} through ${fcf.end}.`;
    const annualFcf = alignAnnualPeriods(annualFacts(companyFacts, ocfTags), annualFacts(companyFacts, capexTags), (left, right) => ({ ...left, val: left.val - Math.abs(right.val), label: "Free cash flow", source_facts: [left, right] }));
...
export function createSecEvidenceClient({ fetchImpl = globalThis.fetch, now = () => Date.now(), userAgent = DEFAULT_USER_AGENT, minRequestIntervalMs = 125, logger = console } = {}) {
  ...
  return {
    ...
    async researchTicker(ticker, { onProgress, budget } = {}) {
      ...
      const filingCapexDiagnostics = [...new Map(filingCapexExtractions.flatMap(({ diagnostics }) => diagnostics).map((item) => [`${item.accession}:${item.table_index}`, item])).values()];
