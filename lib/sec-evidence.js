...
function applyExtractedFindings(fragments, findings, retrievedAt, records) {
  ...
  const orphaned = undatedSplits.filter((item) => (keysByRatio.get(item.ratio) ?? []).length === 0);
  const splitGroups = [...datedSplitGroups.values(), ...orphaned.map((item) => [item])];
