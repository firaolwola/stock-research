...
function applyExtractedFindings(fragments, findings, retrievedAt, records) {
  ...
  for (const [domain, domainFindings] of Object.entries(byDomain)) {
    ...
    domainFindings.forEach((finding, index) => {
      ...
      fragment.claims.push({ id: claimId, text: finding.statement, materiality: ["reverse_split", "going_concern", "accounting_warning", "non_reliance", "exchange_compliance"].includes(finding.kind) ? "high" : "medium", state: finding.evidence_state, as_of: `${findingDate}T00:00:00Z`, source_ids: [sourceId] });
  ...
  for (const item of reverse) {
    ...
    const key = item.ratio && actionDate ? `${item.ratio}:${actionDate}` : `${item.ratio}:${item.action_state}:${item.accession}`;
    if (!splitGroups.has(key)) splitGroups.set(key, []); splitGroups.get(key).push(item);
  }
  const normalizedSplits = [...splitGroups.values()].map((group) => {
    ...
    return { ...preferred, claimIds: [...new Set(group.map((item) => item.claimId))] };
  ...
  if (catalyst && compliance.length) {
    ...
    if (capital && terminalEvidence) {
      ...
      capital.security = { ...capital.security, listing_venue: venue, listing_status: "delisted", evidence_state: "confirmed", claim_ids: [...new Set([...capital.security.claim_ids, terminalEvidence.claimId, ...lineageClaimsAt(terminalEvidence.event_date)])] };
    }
  }
