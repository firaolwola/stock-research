...
import { extractSecFilingEvidence, findMaterialExhibitUrl, normalizeCatalystClassification } from "./sec-filing-extraction.js";
...
function compactFindings(findings) {
  const limits = { reverse_split: 10, offering: 2, warrant: 2, convertible: 2, exchange_compliance: 3 }; const counts = new Map(); const seen = new Set();
...
function financialFragment(identity, companyFacts, retrievedAt, records) {
  const definitions = {
    ...
    profitability: [["us-gaap", "NetIncomeLoss", "Net income or loss"], ["ifrs-full", "ProfitLoss", "Profit or loss"], ["ifrs-full", "ProfitLossAttributableToOwnersOfParent", "Profit or loss attributable to owners"], ["ifrs-full", "ProfitLossFromContinuingOperations", "Profit or loss from continuing operations"], ...(FOREIGN_PROFIT_ALIASES_BY_CIK[identity.cik] ?? [])]
  };
...
function applyExtractedFindings(fragments, findings, retrievedAt, records) {
  ...
  if (catalyst && compliance.length) {
    ...
    catalyst.compliance_and_warnings = { state: "limited_coverage", summary: active.length ? "Selected filings contain an active exchange-compliance condition." : "Selected filings contain resolved historical exchange-compliance evidence; no active deficiency is inferred from it.", coverage_notes: ["Fast does not exhaustively search exchange notices or older filings."], items: compliance.map((item, index) => ({ id: `compliance-${index + 1}`, kind: "exchange_compliance", title: item.title, state: "confirmed", summary: item.statement, event_date: item.event_date, resolution_state: item.resolution_state, claim_ids: [item.claimId, ...lineageClaimsAt(item.event_date)] })), claim_ids: [...new Set(compliance.flatMap((item) => [item.claimId, ...lineageClaimsAt(item.event_date)]))] };
    const terminal = [...compliance].filter((item) => /(?:trading|securities).{0,160}(?:suspend|delist)|(?:commence|began|continue).{0,120}(?:OTCID|OTC Pink|OTC Expert Market)/i.test(item.statement)).sort((a, b) => b.event_date.localeCompare(a.event_date))[0];
    if (capital && terminal) {
      const venue = /OTC Expert Market/i.test(terminal.statement) ? "OTC Expert Market" : /OTC Pink/i.test(terminal.statement) ? "OTC Pink" : /OTCID/i.test(terminal.statement) ? "OTCID" : capital.security.listing_venue;
      capital.security = { ...capital.security, listing_venue: venue, listing_status: "delisted", evidence_state: "confirmed", claim_ids: [...new Set([...capital.security.claim_ids, terminal.claimId, ...lineageClaimsAt(terminal.event_date)])] };
    }
  }
