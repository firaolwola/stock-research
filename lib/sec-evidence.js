...
export function createSecEvidenceClient({ fetchImpl = globalThis.fetch, now = () => Date.now(), userAgent = DEFAULT_USER_AGENT, minRequestIntervalMs = 125, logger = console } = {}) {
  ...
  const cachedJson = async (key, url, ttl, { phase, endpointCategory, requestCounter, format = "json" }) => {
    ...
    const request = requestQueue.then(async () => {
      ...
      try {
        response = await fetchImpl(url, { headers: { "User-Agent": userAgent, Accept: format === "text" ? "text/html,text/plain;q=0.9,*/*;q=0.8" : "application/json", "Accept-Encoding": "gzip, deflate" } });
  ...
  return {
    ...
    async researchTicker(ticker, { onProgress } = {}) {
      ...
      const retrieve = async (key, url, ttl, phase, endpointCategory, format = "json") => {
        try { return await cachedJson(key, url, ttl, { phase, endpointCategory, requestCounter, format }); }
