...
function compactFindings(findings) {
  const limits = { reverse_split: 6, offering: 2, warrant: 2, convertible: 2, exchange_compliance: 3 }; const counts = new Map(); const seen = new Set();
...
function identityFragment(ticker, match, submissions, retrievedAt, records, historicalIdentity = null) {
  ...
  if (historicalIdentity) {
    for (const prior of historicalIdentity.prior_tickers ?? []) {
      ...
      priorIdentities.push({ name: prior.name ?? null, ticker: prior.ticker, effective_from: null, effective_to: prior.effective_to ?? historicalIdentity.source_date, linkage_state: "confirmed", linkage_confidence: "high", claim_ids: [id] });
    }
  }
  ...
  return {
    ...
    security: { ticker, name: submissions.name || match.name, security_type: historicalIdentity?.security_type ?? "unknown", listing_venue: historicalIdentity?.listing_venue ?? match.exchange ?? "Unknown", listing_status: historicalIdentity?.listing_status ?? "unknown", evidence_state: historicalIdentity ? "confirmed" : "limited_coverage", claim_ids: [claimId, ...lineageClaims.map((claim) => claim.id)] },
    ...
    sources: [
      { id: sourceId, title: historicalIdentity ? "SEC-filed ticker and listing lineage" : "SEC ticker, CIK, and exchange associations", url: historicalIdentity?.source_url ?? TICKER_URL, published_date: historicalIdentity?.source_date ?? retrievedAt.slice(0, 10), source_type: historicalIdentity ? "sec_filing" : "other_primary", confidence: "high", retrieved_at: retrievedAt, supported_claim_ids: [claimId, ...(historicalIdentity ? lineageClaims.map((claim) => claim.id) : [])] },
      ...(!historicalIdentity && lineageClaims.length ? [{ id: "source-sec-submissions", title: "SEC submissions metadata", url: `${SEC_BASE}/submissions/CIK${cik}.json`, published_date: retrievedAt.slice(0, 10), source_type: "other_primary", confidence: "high", retrieved_at: retrievedAt, supported_claim_ids: lineageClaims.map((claim) => claim.id) }] : [])
    ]
  };
}
...
function financialFragment(identity, companyFacts, retrievedAt, records) {
  const definitions = {
    ...
    profitability: [["us-gaap", "NetIncomeLoss", "Net income or loss"], ["ifrs-full", "ProfitLoss", "Profit or loss"], ["ifrs-full", "ProfitLossAttributableToOwnersOfParent", "Profit or loss attributable to owners"], ["ifrs-full", "ProfitLossFromContinuingOperations", "Profit or loss from continuing operations"]]
  };
...
function applyExtractedFindings(fragments, findings, retrievedAt, records) {
  ...
  if (catalyst && compliance.length) {
    const active = compliance.filter((item) => item.resolution_state === "active");
    catalyst.compliance_and_warnings = { state: "limited_coverage", summary: active.length ? "Selected filings contain an active exchange-compliance condition." : "Selected filings contain resolved historical exchange-compliance evidence; no active deficiency is inferred from it.", coverage_notes: ["Fast does not exhaustively search exchange notices or older filings."], items: compliance.map((item, index) => ({ id: `compliance-${index + 1}`, kind: "exchange_compliance", title: item.title, state: "confirmed", summary: item.statement, event_date: item.event_date, resolution_state: item.resolution_state, claim_ids: [item.claimId] })), claim_ids: compliance.map((item) => item.claimId) };
  }
  ...
  const financial = fragments.financial; const warnings = byDomain.financial.filter((item) => ["going_concern", "bankruptcy", "accounting_warning", "non_reliance"].includes(item.kind));
  if (financial && warnings.length) {
    ...
    financial.financial_assessment.material_warnings.push(...warnings.map((item, index) => ({ id: `financial-filing-warning-${index + 1}`, kind: ["accounting_warning", "non_reliance"].includes(item.kind) ? "accounting" : item.kind === "bankruptcy" ? "other" : "going_concern", state: "confirmed", severity: item.severity, title: item.title, as_of: item.event_date, summary: item.statement, claim_ids: [item.claimId] })));
...
export function createSecEvidenceClient({ fetchImpl = globalThis.fetch, now = () => Date.now(), userAgent = DEFAULT_USER_AGENT, minRequestIntervalMs = 125, logger = console } = {}) {
  ...
  return {
    ...
    async researchTicker(ticker, { onProgress, budget } = {}) {
      ...
      const documents = (await Promise.all(documentPromises)).filter(Boolean); const findings = documents.flatMap(({ row, url, html }) => extractSecFilingEvidence({ html, form: row.form, filed: row.filed, accession: row.accession, documentUrl: url, documentName: row.document, evaluatedAt: retrievedAt }));
      ...
      for (const exhibitCandidate of exhibitCandidates) {
        ...
        if (exhibit) { cacheStatus.documents.push({ accession: exhibitCandidate.row.accession, document: exhibitName, state: exhibit.cache }); findings.push(...extractSecFilingEvidence({ html: exhibit.data, form: exhibitCandidate.row.form, filed: exhibitCandidate.row.filed, accession: exhibitCandidate.row.accession, documentUrl: exhibitUrl, documentName: exhibitName, evaluatedAt: retrievedAt })); }
      }
