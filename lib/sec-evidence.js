...
import { extractSecFilingEvidenceWithDiagnostics, findMaterialExhibitUrl, normalizeCatalystClassification } from "./sec-filing-extraction.js";
...
function applyExtractedFindings(fragments, findings, retrievedAt, records, corporateActionDiagnostics = []) {
  ...
  normalizedSplits.forEach((item, index) => {
    ...
    for (const claimId of item.claimIds) {
      const finding = reverse.find((candidate) => candidate.claimId === claimId);
      const diagnostic = corporateActionDiagnostics.find((candidate) => candidate.source_accession === finding?.accession && candidate.local_text_span_id === finding?.local_text_span_id);
...
export function createSecEvidenceClient({ fetchImpl = globalThis.fetch, now = () => Date.now(), userAgent = DEFAULT_USER_AGENT, minRequestIntervalMs = 125, logger = console } = {}) {
  ...
  return {
    ...
    async researchTicker(ticker, { onProgress, budget } = {}) {
      ...
      const documents = (await Promise.all(documentPromises)).filter(Boolean); const corporateActionDiagnostics = [];
      const findings = documents.flatMap(({ row, url, html }) => { const extracted = extractSecFilingEvidenceWithDiagnostics({ html, form: row.form, filed: row.filed, accession: row.accession, documentUrl: url, documentName: row.document, evaluatedAt: retrievedAt }); corporateActionDiagnostics.push(...extracted.corporate_action_diagnostics); return extracted.findings; });
      ...
      for (const exhibitCandidate of exhibitCandidates) {
        ...
        if (exhibit) { cacheStatus.documents.push({ accession: exhibitCandidate.row.accession, document: exhibitName, state: exhibit.cache }); const extracted = extractSecFilingEvidenceWithDiagnostics({ html: exhibit.data, form: exhibitCandidate.row.form, filed: exhibitCandidate.row.filed, accession: exhibitCandidate.row.accession, documentUrl: exhibitUrl, documentName: exhibitName, evaluatedAt: retrievedAt }); findings.push(...extracted.findings); corporateActionDiagnostics.push(...extracted.corporate_action_diagnostics); }
      }
      const normalizedCorporateActionDiagnostics = applyExtractedFindings(Object.fromEntries(Object.entries(results).map(([key, value]) => [key, value.fragment])), compactFindings(findings), retrievedAt, records, corporateActionDiagnostics);
