...
import { extractFilingCapitalExpenditureFacts, extractSecFilingEvidenceWithDiagnostics, filingHtmlToText, findMaterialExhibitUrls, normalizeCatalystClassification } from "./sec-filing-extraction.js";
...
function buildInterimContext(observations, label) {
  if (!Array.isArray(observations) || observations.length < 2) return null;
  const previous = observations.at(-2); const latest = observations.at(-1);
  const valueOf = (item) => item?.value ?? item?.val;
  const startOf = (item) => item?.period_start ?? item?.start ?? item?.end;
  const endOf = (item) => item?.period_end ?? item?.end;
  if (![previous, latest].every((item) => Number.isFinite(valueOf(item)) && item.unit === latest.unit)) return null;
  const delta = valueOf(latest) - valueOf(previous);
  const trend = delta > 0 ? "improving" : delta < 0 ? "deteriorating" : "stable";
  const level = valueOf(latest) < 0 ? "remains negative" : valueOf(latest) > 0 ? "is positive" : "is zero";
  return {
    state: "confirmed", trend, latest_value: valueOf(latest), comparison_value: valueOf(previous), unit: latest.unit,
    latest_period_start: startOf(latest), latest_period_end: endOf(latest),
    comparison_period_start: startOf(previous), comparison_period_end: endOf(previous),
    summary: `Latest comparable interim ${label} ${level} at ${valueOf(latest)} ${latest.unit} for ${startOf(latest)} through ${endOf(latest)}; ${trend} versus ${startOf(previous)} through ${endOf(previous)} (${valueOf(previous)} ${previous.unit}). This is freshness context and does not replace the annual primary trend.`,
    claim_ids: [...new Set([...(previous.claim_ids ?? []), ...(latest.claim_ids ?? [])])]
  };
}
...
function factCandidates(companyFacts, tags) {
  return tags.flatMap(([taxonomy, tag, label]) => {
    ...
    return Object.entries(concept.units ?? {}).flatMap(([unit, facts]) => facts
      ...
      .map((fact) => ({ ...fact, unit, label: concept.label || label, taxonomy, tag, ...(tag.toLowerCase().includes("intangible") ? { capital_expenditure_type: "intangible_assets" } : {}), ...(taxonomy === "sec-filing" && tag === "CapitalExpenditures" && fact.capital_expenditure_component ? {} : { capital_expenditure_component: `${taxonomy}:${tag}` }) })));
  }).sort((a, b) => `${b.end}:${b.filed}`.localeCompare(`${a.end}:${a.filed}`));
}

function aggregateCapitalExpenditureFacts(facts) {
  const grouped = new Map();
  const unitsByPeriod = new Map();
  for (const fact of facts) {
    const periodKey = `${fact.start ?? fact.end}:${fact.end}`;
    const units = unitsByPeriod.get(periodKey) ?? new Set();
    units.add(fact.unit);
    unitsByPeriod.set(periodKey, units);
    const key = `${fact.unit}:${periodKey}`;
    const type = fact.capital_expenditure_type ?? "property_plant_equipment";
    const component = fact.capital_expenditure_component ?? `${fact.taxonomy ?? "unknown"}:${fact.tag ?? type}`;
    const group = grouped.get(key) ?? { periodKey, factsByComponent: new Map(), conflict: false };
    const existing = group.factsByComponent.get(component);
    if (!existing) group.factsByComponent.set(component, fact);
    else if (Math.abs(existing.val) !== Math.abs(fact.val)) group.conflict = true;
    else if (String(fact.filed) > String(existing.filed)) group.factsByComponent.set(component, fact);
    grouped.set(key, group);
  }
  return [...grouped.entries()].filter(([, group]) => !group.conflict && (unitsByPeriod.get(group.periodKey)?.size ?? 0) === 1).map(([, group]) => {
    const sourceFacts = [...group.factsByComponent.values()];
    // A filing may show a generic total alongside its component rows. The
    // total is already the issuer's aggregate and must win rather than being
    // added to the components a second time. Keep component rows out of the
    // derived value to avoid overstating cash outflows.
    const total = sourceFacts.find((fact) => fact.capital_expenditure_type === "total_capital_expenditure");
    if (total) return { ...total, val: Math.abs(total.val), label: "Capital expenditures", capital_expenditure_type: "total_capital_expenditure", source_facts: [total] };
    const types = [...new Set(sourceFacts.map((fact) => fact.capital_expenditure_type ?? "property_plant_equipment"))];
    return { ...sourceFacts[0], val: sourceFacts.reduce((sum, fact) => sum + Math.abs(fact.val), 0), label: "Capital expenditures", capital_expenditure_type: types.length > 1 ? "mixed_capital_assets" : types[0], source_facts: sourceFacts };
  });
}

function annualFactsFromList(facts) {
  const candidates = facts.filter((fact) => ["10-K", "20-F", "40-F"].includes(fact.form) && (!fact.start || Math.abs(Math.round((new Date(fact.end) - new Date(fact.start)) / 86_400_000) - 365) <= 35));
  const periods = new Map();
  for (const fact of candidates) {
    const key = `${fact.unit}:${fact.end}`; const existing = periods.get(key);
...
function alignedFactPairs(leftFacts, rightFacts) {
  ...
  return uniqueByPeriod(leftFacts).flatMap((left) => {
    const right = rightByPeriod.get(`${left.unit}:${left.start ?? left.end}:${left.end}`);
    return right ? [{ left, right }] : [];
  }).sort((a, b) => {
    const endOrder = b.left.end.localeCompare(a.left.end);
    if (endOrder) return endOrder;
    const duration = (fact) => fact.start ? Math.round((new Date(fact.end) - new Date(fact.start)) / 86_400_000) : 0;
    return duration(a.left) - duration(b.left) || String(b.left.filed).localeCompare(String(a.left.filed));
  });
}

function alignDerivedPeriods(leftFacts, rightFacts, derive) {
  const aligned = alignedFactPairs(leftFacts, rightFacts).map(({ left, right }) => derive(left, right));
...
function withFilingCapexFacts(companyFacts, filingCapexFacts) {
  const merged = structuredClone(companyFacts ?? { facts: {} }); merged.facts ??= {};
  if (!filingCapexFacts?.length) return merged;
  const units = {};
  for (const fact of filingCapexFacts) (units[fact.unit] ??= []).push({ ...fact });
  merged.facts["sec-filing"] = { ...(merged.facts["sec-filing"] ?? {}), CapitalExpenditures: { label: "Capital expenditures", units } };
  return merged;
}

function companyFactsFlowCurrency(companyFacts) {
  const tags = [["us-gaap", "NetCashProvidedByUsedInOperatingActivities", "Operating cash flow"], ["ifrs-full", "CashFlowsFromUsedInOperatingActivities", "Operating cash flow"]];
  const candidates = factCandidates(companyFacts, tags);
  if (!candidates.length) return null;
  // A filer may retain historical facts in a prior presentation currency or
  // expose both GAAP/IFRS concepts. Use only the newest filed OCF period when
  // supplying a currency hint to a filing-table capex extractor. Require all
  // same-period candidates to agree; otherwise the hint remains unknown.
  const newestEnd = candidates[0].end;
  const newestFiled = candidates.filter((fact) => fact.end === newestEnd).map((fact) => fact.filed).sort().at(-1);
  const units = [...new Set(candidates.filter((fact) => fact.end === newestEnd && fact.filed === newestFiled).map((fact) => fact.unit).filter((unit) => /^[A-Z]{3}$/.test(unit)))];
  return units.length === 1 ? units[0] : null;
}

function financialFragment(identity, companyFacts, retrievedAt, records, { filingCapexFacts = [], filingCapexDiagnostics = [] } = {}) {
  const definitions = {
  ...
  const normalizationDiagnostics = [...rejectedProfitConceptDiagnostics(companyFacts, identity, definitions.profitability), ...filingCapexDiagnostics];
  const claims = []; const metrics = {}; const sources = [];
  const addFact = (key, fact, { label = fact?.label, value = fact?.val, state = "confirmed", summary, comparable = [fact], annual = [], interimContext = null } = {}) => {
    const fromFilingTable = fact?.source_type === "sec_filing_table";
    const claimId = `claim-financial-${key}`;
    const sourceFacts = (item) => item?.source_facts?.length ? item.source_facts.flatMap((sourceFact) => sourceFacts(sourceFact)) : [item];
    const sourceDetails = (item, fallbackId) => sourceFacts(item).filter(Boolean).map((sourceFact, index) => ({
      id: sourceFact.source_id ?? (index ? `${fallbackId}-${index + 1}` : fallbackId),
      url: sourceFact.source_url ?? `${SEC_BASE}/api/xbrl/companyfacts/CIK${identity.cik}.json`,
      title: sourceFact.source_title ?? `SEC Company Facts — ${label}`,
      type: sourceFact.source_type === "sec_filing_table" ? "sec_filing" : "other_primary",
      published: sourceFact.filed ?? fact?.filed
    }));
    const factSourceDetails = sourceDetails(fact, `source-sec-companyfacts-${key}`);
    const sourceId = factSourceDetails[0]?.id ?? `source-sec-companyfacts-${key}`;
    const text = summary ?? `${fromFilingTable ? "SEC filing table" : "SEC Company Facts"} reports ${label} of ${value} ${fact.unit} for the period ending ${fact.end}, filed ${fact.filed}.`;
    const claimSourceIds = factSourceDetails.map((detail) => detail.id);
    claims.push({ id: claimId, text, materiality: key === "cash" || key === "debt" ? "high" : "medium", state, as_of: `${fact.filed}T00:00:00Z`, source_ids: claimSourceIds });
    for (const detail of factSourceDetails) {
      const existing = sources.find((source) => source.id === detail.id);
      if (existing) existing.supported_claim_ids = [...new Set([...existing.supported_claim_ids, claimId])];
      else sources.push({ id: detail.id, title: detail.title, url: detail.url, published_date: detail.published, source_type: detail.type, confidence: "high", retrieved_at: retrievedAt, supported_claim_ids: [claimId] });
    }
    const buildObservations = (items, suffix) => state === "confirmed" ? items.map((item, index) => {
      ...
      const observationClaimId = `${claimId}-${suffix}-${index + 1}`;
      const observationDetails = sourceDetails(item, `${sourceId}-${suffix}-${index + 1}`);
      const observationSourceId = observationDetails[0]?.id ?? `${sourceId}-${suffix}-${index + 1}`;
      const observationFromFiling = item.source_type === "sec_filing_table" || item.source_facts?.some((sourceFact) => sourceFact.source_type === "sec_filing_table");
      const observationText = `${observationFromFiling ? "SEC filing table" : "SEC Company Facts"} reports ${label} of ${item.val} ${item.unit} for the comparable period ending ${item.end}, filed ${item.filed}.`;
      claims.push({ id: observationClaimId, text: observationText, materiality: "medium", state: "confirmed", as_of: `${item.filed}T00:00:00Z`, source_ids: observationDetails.map((detail) => detail.id) });
      for (const detail of observationDetails) {
        const existing = sources.find((source) => source.id === detail.id);
        if (existing) existing.supported_claim_ids = [...new Set([...existing.supported_claim_ids, observationClaimId])];
        else sources.push({ id: detail.id, title: detail.title, url: detail.url, published_date: detail.published, source_type: detail.type, confidence: "high", retrieved_at: retrievedAt, supported_claim_ids: [observationClaimId] });
      }
    ...
    const resolvedInterimContext = interimContext
      ? {
          ...interimContext,
          claim_ids: [...new Set(observations
            .filter((item) => [interimContext.latest_period_end, interimContext.comparison_period_end].includes(item.period_end))
            .flatMap((item) => item.claim_ids ?? []))]
        }
      : null;
    metrics[key] = state === "confirmed"
      ? { state, label, value, unit: fact.unit, period_start: fact.start || fact.end, period_end: fact.end, trend: "unknown", comparison_period_start: comparison?.period_start ?? null, comparison_period_end: comparison?.period_end ?? null, observations, annual_observations: annualObservations, ...(resolvedInterimContext ? { interim_context: resolvedInterimContext } : {}), summary: text, claim_ids: [...new Set([...observations, ...annualObservations].flatMap((item) => item.claim_ids))] }
  ...
  const capexTags = [["us-gaap", "PaymentsToAcquirePropertyPlantAndEquipment", "Capital expenditures"], ["ifrs-full", "PurchaseOfPropertyPlantAndEquipment", "Capital expenditures"], ["us-gaap", "PaymentsToAcquireIntangibleAssets", "Capital expenditures (intangible assets)"], ["ifrs-full", "PurchaseOfIntangibleAssets", "Capital expenditures (intangible assets)"], ["sec-filing", "CapitalExpenditures", "Capital expenditures"]];
  const ocf = selectUnambiguousFact(companyFacts, ocfTags).fact;
  // Select FCF from the newest aligned OCF/capex pair, rather than requiring
  // the independently selected facts to share a period. SEC filers commonly
  // report a newest quarterly OCF alongside YTD capex; the aligned YTD pair
  // is valid and should not be discarded merely because the visible OCF metric
  // prefers the shorter quarter.
  const capexFacts = aggregateCapitalExpenditureFacts(factCandidates(companyFacts, capexTags));
  const fcfPairs = alignedFactPairs(factCandidates(companyFacts, ocfTags), capexFacts);
  const fcfPair = fcfPairs[0] ?? null;
  ...
  if (fcfPair) {
    const { left: alignedOcf, right: alignedCapex } = fcfPair;
    const fcf = { ...alignedOcf, val: alignedOcf.val - Math.abs(alignedCapex.val), label: "Free cash flow", source_facts: [alignedOcf, alignedCapex] };
    const comparableFcf = alignDerivedPeriods(factCandidates(companyFacts, ocfTags), capexFacts, (left, right) => ({ ...left, val: left.val - Math.abs(right.val), label: "Free cash flow", source_facts: [left, right] }));
    const capexDescription = alignedCapex.capital_expenditure_type === "intangible_assets" ? "qualifying intangible-asset purchases" : alignedCapex.capital_expenditure_type === "mixed_capital_assets" ? "property/equipment and qualifying intangible-asset purchases" : "capital expenditures";
    const summary = `Free cash flow of ${fcf.val} ${fcf.unit} is calculated as operating cash flow ${alignedOcf.val} less ${capexDescription} ${Math.abs(alignedCapex.val)} for ${fcf.start || fcf.end} through ${fcf.end}.`;
    const annualFcf = alignAnnualPeriods(annualFacts(companyFacts, ocfTags), annualFactsFromList(capexFacts), (left, right) => ({ ...left, val: left.val - Math.abs(right.val), label: "Free cash flow", source_facts: [left, right] }));
    addFact("free_cash_flow", fcf, { summary, comparable: comparableFcf, annual: annualFcf, interimContext: buildInterimContext(comparableFcf, "free cash flow") });
  ...
  for (const key of ["cash", "debt", "free_cash_flow", "cash_burn"]) {
    ...
    metric.state = "limited_coverage"; metric.value = null; metric.unit = null; metric.trend = "unknown"; metric.observations = []; metric.annual_observations = []; delete metric.interim_context;
...
function promoteIssuerReportingProperties(capital, documents, retrievedAt, records) {
  ...
  const propertyResolution = resolveReportingPropertyCandidates(texts.map((item) => ({ ...item, cik: capital.issuer.cik, ticker: capital.security.ticker })));
  const ifrs = propertyResolution.accounting.state === "confirmed" && propertyResolution.accounting.value === "IFRS";
  const usGaap = propertyResolution.accounting.state === "confirmed" && propertyResolution.accounting.value === "US_GAAP";
  ...
  const directShares = propertyResolution.security.state === "confirmed" && propertyResolution.security.value === "direct_share";
  const ads = propertyResolution.security.state === "confirmed" && propertyResolution.security.value === "ads";
  const venues = propertyResolution.additional_listing_venues;
  ...
  const accountingCandidate = propertyResolution.accounting.candidate;
  const securityCandidate = propertyResolution.security.candidate;
  const sourceFor = (candidate) => texts.find((item) => item.row.accession === candidate?.accession) ?? source;
  const promotedClaims = [];
  const addPropertyClaim = (suffix, fact, propertySource, target) => {
    if (!fact || !propertySource) return;
    const claimId = `claim-sec-reporting-${suffix}`; const sourceId = `source-sec-reporting-${suffix}`;
    const text = `SEC-filed authoritative evidence identifies ${capital.issuer.legal_name} with ${fact}.`;
    capital.claims.push({ id: claimId, text, materiality: "high", state: "confirmed", as_of: `${propertySource.row.filed}T00:00:00Z`, source_ids: [sourceId] });
    capital.sources.push({ id: sourceId, title: `${propertySource.row.form} filed ${propertySource.row.filed} — ${fact}`, url: propertySource.url, published_date: propertySource.row.filed, source_type: "sec_filing", confidence: "high", retrieved_at: retrievedAt, supported_claim_ids: [claimId] });
    records.push(evidenceRecord({ id: `evidence-sec-reporting-${suffix}`, category: "security_and_listing", date: propertySource.row.filed, text, sourceId, cik: capital.issuer.cik, ticker: capital.security.ticker }));
    promotedClaims.push({ claimId, target });
  };
  addPropertyClaim("regime", foreign ? `filing regime ${filingRegime}` : null, source, "issuer");
  addPropertyClaim("accounting", ifrs ? "IFRS as issued by the IASB" : usGaap ? "U.S. GAAP" : null, sourceFor(accountingCandidate), "issuer");
  addPropertyClaim("security", directShares ? `direct common shares${venues.length ? ` with ${venues.join(" and ")} venue evidence` : ""}` : ads ? "American Depositary Shares" : null, sourceFor(securityCandidate), "security");
  const issuerClaimIds = promotedClaims.filter((item) => item.target === "issuer").map((item) => item.claimId);
  const securityClaimIds = promotedClaims.filter((item) => item.target === "security").map((item) => item.claimId);
  capital.issuer = { ...capital.issuer, jurisdiction: jurisdiction ?? capital.issuer.jurisdiction ?? null, foreign_private_issuer: foreign ? true : filingRegime.startsWith("domestic") ? false : capital.issuer.foreign_private_issuer ?? null, filing_regime: filingRegime !== "unknown" ? filingRegime : capital.issuer.filing_regime ?? "unknown", accounting_basis: ifrs ? "IFRS" : usGaap ? "US_GAAP" : capital.issuer.accounting_basis ?? "unknown", accounting_authority: ifrs ? "IASB" : usGaap ? "FASB/SEC U.S. GAAP" : capital.issuer.accounting_authority ?? null, presentation_currency: currency, claim_ids: [...new Set([...capital.issuer.claim_ids, ...issuerClaimIds])] };
  capital.security = { ...capital.security, security_structure: directShares ? "direct_share" : ads ? "ads" : "unknown", depositary_ratio: null, additional_listing_venues: venues, security_type: directShares && capital.security.security_type === "unknown" ? "foreign_ordinary_share" : capital.security.security_type, claim_ids: [...new Set([...capital.security.claim_ids, ...securityClaimIds])] };
  return [{ foreign_filer_source: foreign ? source.row.accession : null, filing_regime_source: filingRegime !== "unknown" ? source.row.accession : null, accounting_framework_source: accountingCandidate?.accession ?? null, accounting_framework_text_range: accountingCandidate?.source_span ?? null, accounting_framework: capital.issuer.accounting_basis, accounting_framework_state: propertyResolution.accounting.state, security_structure_source: securityCandidate?.accession ?? null, security_structure_state: propertyResolution.security.state, primary_venue: capital.security.listing_venue, additional_venues: venues, candidates: propertyResolution.candidates, promoted_properties: { foreign_private_issuer: capital.issuer.foreign_private_issuer, filing_regime: filingRegime, accounting_basis: capital.issuer.accounting_basis, accounting_authority: capital.issuer.accounting_authority, presentation_currency: currency, security_structure: capital.security.security_structure, depositary_ratio: null } }];
}
...
function applyExtractedFindings(fragments, findings, retrievedAt, records, corporateActionDiagnostics = [], listingComplianceDiagnostics = []) {
  ...
  if (catalyst && compliance.length) {
    const expandedCompliance = compliance.flatMap((item) => {
      const rules = complianceRuleIds(item.statement);
      if (rules.length <= 1) return [item];
      return rules.map((rule) => {
        const statement = projectComplianceStatement(item.statement, rule);
        const claimId = `${item.claimId}-${rule.includes("a_2") ? "bid" : "equity"}`;
        catalyst.claims.push({ id: claimId, text: statement, materiality: "high", state: item.evidence_state, as_of: `${item.event_date ?? item.source_filing_date}T00:00:00Z`, source_ids: [item.sourceId] });
        const source = catalyst.sources.find((candidate) => candidate.id === item.sourceId);
        if (source) source.supported_claim_ids = [...new Set([...source.supported_claim_ids, claimId])];
        records.push(evidenceRecord({ id: `evidence-${claimId}`, category: "catalysts_news", state: item.evidence_state, date: item.event_date ?? item.source_filing_date, text: statement, sourceId: item.sourceId, cik: catalyst.identity.cik, ticker: catalyst.identity.ticker }));
        return { ...item, statement, claimId, normalized_compliance_rule: rule };
      });
    });
...
export function createSecEvidenceClient({ fetchImpl = globalThis.fetch, now = () => Date.now(), userAgent = DEFAULT_USER_AGENT, minRequestIntervalMs = 125, logger = console } = {}) {
  ...
  return {
    ...
    async researchTicker(ticker, { onProgress, budget } = {}) {
      ...
      const factsData = factsResult?.data ?? { cik, entityName: submissions.name, facts: {} };
      if (factsResult) { assertIssuerIdentity(cik, submissions, factsResult.data); cacheStatus.companyfacts = factsResult.cache; }
      const documents = (await Promise.all(documentPromises)).filter(Boolean); const reportingDocuments = [...documents]; const corporateActionDiagnostics = []; const ntFilingDiagnostics = [...ntSelection.diagnostics];
      ...
      // Annual foreign-filer submissions commonly keep the audited statements
      // in a linked 99.x exhibit (for example, a 40-F's ex-992... document).
      // Inspect only a small, SEC-hosted allowlist so Fast remains bounded and
      // does not become open-ended filing crawling.
      const exhibitCandidates = documents.flatMap(({ row, url, html }) => {
        const annualReportingForm = /^(?:20-F|40-F)$/.test(row.form);
        const eligible = annualReportingForm || (CATALYST_FORMS.has(row.form) && (/(?:3\.01|4\.02|5\.03)/.test(row.items) || !findings.some((finding) => finding.accession === row.accession && ["catalyst", "reverse_split", "non_reliance", "exchange_compliance"].includes(finding.kind))));
        if (!eligible) return [];
        return findMaterialExhibitUrls(html, url, { limit: annualReportingForm ? 3 : 1 }).map((exhibitUrl) => ({ row, url, html, exhibitUrl }));
      }).slice(0, 4);
      for (const exhibitCandidate of exhibitCandidates) {
        if (budget?.isStopped()) break;
        const exhibitUrl = exhibitCandidate.exhibitUrl; const exhibitName = new URL(exhibitUrl).pathname.split("/").at(-1);
        ...
        if (exhibit) { cacheStatus.documents.push({ accession: exhibitCandidate.row.accession, document: exhibitName, state: exhibit.cache }); reportingDocuments.push({ row: { ...exhibitCandidate.row, document: exhibitName }, url: exhibitUrl, html: exhibit.data }); const extracted = extractSecFilingEvidenceWithDiagnostics({ html: exhibit.data, form: exhibitCandidate.row.form, filed: exhibitCandidate.row.filed, reportDate: exhibitCandidate.row.reportDate, accession: exhibitCandidate.row.accession, documentUrl: exhibitUrl, documentName: exhibitName, evaluatedAt: retrievedAt }); findings.push(...extracted.findings); corporateActionDiagnostics.push(...extracted.corporate_action_diagnostics); ntFilingDiagnostics.push(...extracted.nt_filing_diagnostics); }
      }
      const filingCurrencyHint = companyFactsFlowCurrency(factsData);
      const filingCapexExtractions = reportingDocuments.map(({ row, url, html }) => extractFilingCapitalExpenditureFacts({ html, form: row.form, filed: row.filed, reportDate: row.reportDate, accession: row.accession, documentUrl: url, documentName: row.document, currencyHint: filingCurrencyHint }));
      const filingCapexFacts = filingCapexExtractions.flatMap(({ facts }) => facts);
      const filingCapexDiagnostics = [...new Map(filingCapexExtractions.flatMap(({ diagnostics }) => diagnostics).map((item) => [`${item.accession}:${item.table_index}:${item.row_index ?? 0}`, item])).values()];
      results.financial = { fragment: financialFragment(capitalBase.identity, withFilingCapexFacts(factsData, filingCapexFacts), retrievedAt, records, { filingCapexFacts, filingCapexDiagnostics }) };
      const reportingIdentityDiagnostics = promoteIssuerReportingProperties(results.capital.fragment, reportingDocuments, retrievedAt, records);
