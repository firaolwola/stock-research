...
export function boundedDocumentRows(rows, retrievedAt) {
  ...
  const groups = [
    ...
    rows.filter((row) => /^(NT 10-K|NT 20-F|NT 40-F)$/.test(row.form)).slice(0, 1),
  ...
  return {
    ...
    async researchTicker(ticker, { onProgress, budget } = {}) {
      ...
      for (const exhibitCandidate of exhibitCandidates) {
        ...
        if (exhibit) { cacheStatus.documents.push({ accession: exhibitCandidate.row.accession, document: exhibitName, state: exhibit.cache }); const extracted = extractSecFilingEvidenceWithDiagnostics({ html: exhibit.data, form: exhibitCandidate.row.form, filed: exhibitCandidate.row.filed, reportDate: exhibitCandidate.row.reportDate, accession: exhibitCandidate.row.accession, documentUrl: exhibitUrl, documentName: exhibitName, evaluatedAt: retrievedAt }); findings.push(...extracted.findings); corporateActionDiagnostics.push(...extracted.corporate_action_diagnostics); }
      }
