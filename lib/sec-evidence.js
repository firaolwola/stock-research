...
function identityFragment(ticker, match, submissions, retrievedAt, records) {
  ...
  const lineageClaims = [];
  const priorIdentities = (submissions.formerNames ?? []).filter((item) => item.name && item.from && item.to).slice(0, 3).map((item, index) => {
...
export function createSecEvidenceClient({ fetchImpl = globalThis.fetch, now = () => Date.now(), userAgent = DEFAULT_USER_AGENT, minRequestIntervalMs = 125 } = {}) {
  ...
  const cachedJson = async (key, url, ttl) => {
    ...
    const request = requestQueue.then(async () => { const wait = Math.max(0, nextRequestAt - now()); if (wait) await new Promise((resolve) => setTimeout(resolve, wait)); nextRequestAt = now() + minRequestIntervalMs;
      const response = await fetchImpl(url, { headers: { "User-Agent": userAgent, Accept: "application/json", "Accept-Encoding": "gzip, deflate" } });
      if (!response.ok) throw Object.assign(new Error("SEC retrieval failed"), { status: response.status }); const data = await response.json(); cache.set(key, { data, expiresAt: now() + ttl }); return data; });
  ...
  return {
    ...
    async researchTicker(ticker, { onProgress } = {}) {
      const started = now(); const retrievedAt = new Date(now()).toISOString(); const records = []; const cacheStatus = {}; let firstUseful = null; const results = {};
      let tickerResult; try { tickerResult = await cachedJson("tickers", TICKER_URL, CACHE_TTL_MS.tickers); cacheStatus.tickers = tickerResult.cache; } catch { return { report: assembleFastReport(ticker, {}), operations: { ...buildResearchOperations({ stage: "fast", latencyMs: now() - started, domains: Object.fromEntries(Object.keys(FAST_DOMAINS).map((key) => [key, { status: "pending" }])) }), retrieval: { status: "unavailable", sec_request_count: 1 } }, evidence_records: [], synthesis: { status: "unavailable" } }; }
      ...
      const submissionResult = await cachedJson(`submissions:${cik}`, `${SEC_BASE}/submissions/CIK${cik}.json`, CACHE_TTL_MS.issuer).catch(() => null); if (submissionResult) cacheStatus.submissions = submissionResult.cache;
      ...
      const operations = () => buildResearchOperations({ stage: "fast", latencyMs: now() - started, firstUsefulLatencyMs: firstUseful, usage: null, webSearchCalls: 0, domains: Object.fromEntries(Object.keys(FAST_DOMAINS).map((key) => [key, { status: results[key]?.fragment ? "completed" : "pending" }] )) });
      await publish(false);
      const factsPromise = cachedJson(`facts:${cik}`, `${SEC_BASE}/api/xbrl/companyfacts/CIK${cik}.json`, CACHE_TTL_MS.issuer).catch(() => null);
      ...
      return { report: assembleFastReport(ticker, results, { generatedAt: retrievedAt }), operations: { ...operations(true), retrieval: { status: "completed", sec_request_count: packet.sec_request_count, cache: cacheStatus } }, evidence_records: records, evidence_packet: packet, synthesis: { status: "pending" } };
    }
  };
}
