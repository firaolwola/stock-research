...
function financialFragment(identity, companyFacts, retrievedAt, records) {
  ...
  const addFact = (key, fact, { label = fact?.label, value = fact?.val, state = "confirmed", summary } = {}) => {
    ...
    metrics[key] = state === "confirmed"
      ? { state, label, value, unit: fact.unit, period_start: fact.start || fact.end, period_end: fact.end, trend: "unknown", comparison_period_start: null, comparison_period_end: null, summary: text, claim_ids: [claimId] }
  ...
  const ocf = selectUnambiguousFact(companyFacts, [["us-gaap", "NetCashProvidedByUsedInOperatingActivities", "Operating cash flow"], ["ifrs-full", "CashFlowsFromUsedInOperatingActivities", "Operating cash flow"]]).fact;
  const capex = selectUnambiguousFact(companyFacts, [["us-gaap", "PaymentsToAcquirePropertyPlantAndEquipment", "Capital expenditures"], ["ifrs-full", "PurchaseOfPropertyPlantAndEquipment", "Capital expenditures"]]).fact;
  if (ocf && capex && ocf.unit === capex.unit && ocf.start === capex.start && ocf.end === capex.end) {
    ...
    addFact("free_cash_flow", fcf, { summary });
    addFact("cash_burn", { ...fcf, val: Math.max(0, -fcf.val), label: "Cash burn based on free cash flow" });
  } else {
  ...
  if (currentDebt && noncurrentDebt && currentDebt.unit === noncurrentDebt.unit && currentDebt.end === noncurrentDebt.end) {
    ...
    addFact("debt", debt, { summary: `Total debt of ${debt.val} ${debt.unit} is calculated from current debt ${currentDebt.val} plus non-current debt ${noncurrentDebt.val} at ${debt.end}.` });
  } else {
  ...
  for (const key of ["cash", "debt", "free_cash_flow", "cash_burn"]) {
    ...
    metric.state = "limited_coverage"; metric.value = null; metric.unit = null; metric.trend = "unknown";
