...
export function createSecEvidenceClient({ fetchImpl = globalThis.fetch, now = () => Date.now(), userAgent = DEFAULT_USER_AGENT, minRequestIntervalMs = 125, logger = console } = {}) {
  ...
  return {
    ...
    async researchTicker(ticker, { onProgress, budget } = {}) {
      ...
      const rows = [...new Map([...filingRows(submissions), ...historicalRows].map((row) => [row.accession, row])).values()]; results.capital = { fragment: capitalFromFilings(capitalBase, rows, retrievedAt, records) }; results.catalyst = { fragment: catalystFragment(capitalBase.identity, rows, retrievedAt, records) }; await publish(false);
