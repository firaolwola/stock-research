...
function boundedDocumentRows(rows, retrievedAt) {
  ...
  const groups = [
    ...
    rows.filter((row) => new Date(row.filed) >= history && (/(?:3\.01|4\.02|5\.03)/.test(row.items) || /split|non-reliance|restatement/i.test(row.description))).slice(0, 3)
  ];
  return [...new Map(groups.flat().map((row) => [row.accession, row])).values()].slice(0, 6);
}
...
function selectUnambiguousFact(companyFacts, tags) {
  ...
  if (!candidates.length) return { fact: null, reason: "missing" };
  ...
  if (!valid.length) return { fact: null, reason: "conflicting", candidates: peers };
  const duration = (fact) => fact.start ? Math.round((new Date(fact.end) - new Date(fact.start)) / 86_400_000) : 0;
  // A quarter and a YTD fact ending on the same date are distinct valid periods,
  // not conflicting values. Prefer the shortest positive duration for the
  // current flow observation; point-in-time facts remain duration zero.
  valid.sort((a, b) => duration(a) - duration(b) || String(b.frame ?? "").localeCompare(String(a.frame ?? "")));
  return { fact: valid[0], reason: null };
}
...
function financialFragment(identity, companyFacts, retrievedAt, records) {
  ...
  for (const key of ["cash", "revenue", "profitability"]) {
    ...
    addFact(key, fact, { comparable: comparableFacts(companyFacts, definitions[key]), annual: annualFacts(companyFacts, definitions[key]) });
  }
...
function applyExtractedFindings(fragments, findings, retrievedAt, records) {
  ...
  for (const [domain, domainFindings] of Object.entries(byDomain)) {
    ...
    domainFindings.forEach((finding, index) => {
      ...
      fragment.claims.push({ id: claimId, text: finding.statement, materiality: ["reverse_split", "going_concern", "accounting_warning", "non_reliance", "exchange_compliance"].includes(finding.kind) ? "high" : "medium", state: finding.evidence_state, as_of: `${finding.event_date}T00:00:00Z`, source_ids: [sourceId] });
  ...
  if (capital && reverse.length) capital.reverse_splits = { state: "limited_coverage", summary: "Authoritative filings distinguish completed reverse splits from proposed or authorized actions.", coverage_notes: ["Fast opens a bounded set of corporate-action filings and does not prove a complete five-year history."], items: reverse.map((item, index) => ({ id: `reverse-split-${index + 1}`, kind: "reverse_split", title: item.title, state: "confirmed", summary: item.statement, event_date: item.event_date, corporate_action_state: item.action_state, claim_ids: [item.claimId] })), claim_ids: reverse.map((item) => item.claimId) };
  ...
  const financial = fragments.financial; const warnings = byDomain.financial.filter((item) => ["going_concern", "accounting_warning", "non_reliance"].includes(item.kind));
  if (financial && warnings.length) {
    ...
    financial.financial_assessment.material_warnings.push(...warnings.map((item, index) => ({ id: `financial-filing-warning-${index + 1}`, kind: ["accounting_warning", "non_reliance"].includes(item.kind) ? "accounting" : "going_concern", state: "confirmed", severity: item.severity, title: item.title, as_of: item.event_date, summary: item.statement, claim_ids: [item.claimId] })));
