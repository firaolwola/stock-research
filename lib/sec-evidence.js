...
import { extractSecFilingEvidence, findMaterialExhibitUrl } from "./sec-filing-extraction.js";
...
const unknownMetric = (label) => ({ state: "unknown", label, value: null, unit: null, period_start: null, period_end: null, trend: "unknown", comparison_period_start: null, comparison_period_end: null, observations: [], summary: `${label} was not established by the available SEC Company Facts.`, claim_ids: [] });
...
function boundedDocumentRows(rows, retrievedAt) {
  ...
  const groups = [
    ...
    rows.filter((row) => /^(10-K|20-F|10-Q|40-F)$/.test(row.form)).slice(0, 1),
    rows.filter((row) => new Date(row.filed) >= history && /(?:3\.01|4\.02|5\.03)/.test(row.items)).slice(0, 1)
  ];
  return [...new Map(groups.flat().map((row) => [row.accession, row])).values()].slice(0, 4);
}

function compactFindings(findings) {
  const limits = { offering: 2, warrant: 2, convertible: 2 }; const counts = new Map(); const seen = new Set();
  return findings.filter((finding) => {
    const signature = `${finding.kind}:${finding.statement.toLowerCase().replace(/[^a-z0-9]+/g, " ").slice(0, 180)}`; if (seen.has(signature)) return false;
    const count = counts.get(finding.kind) ?? 0; if (count >= (limits[finding.kind] ?? 1)) return false;
    seen.add(signature); counts.set(finding.kind, count + 1); return true;
  });
}
...
function selectUnambiguousFact(companyFacts, tags) {
  ...
  if (!newest) return { fact: null, reason: "missing" };
  const peers = candidates.filter((fact) => fact.end === newest.end && fact.filed === newest.filed);
  if (new Set(peers.map((fact) => `${fact.unit}:${fact.val}`)).size > 1) return { fact: null, reason: "conflicting", candidates: peers };
  return { fact: newest, reason: null };
}
...
function identityFragment(ticker, match, submissions, retrievedAt, records) {
  ...
  return {
    ...
    security: { ticker, name: submissions.name || match.name, security_type: "unknown", listing_venue: match.exchange || "Unknown", listing_status: "unknown", evidence_state: "limited_coverage", claim_ids: [claimId] },
...
function financialFragment(identity, companyFacts, retrievedAt, records) {
  ...
  const addFact = (key, fact, { label = fact?.label, value = fact?.val, state = "confirmed", summary, comparable = [fact] } = {}) => {
    ...
    const observations = state === "confirmed" ? comparable.map((item, index) => {
      ...
      const observationClaimId = `${claimId}-observation-${index + 1}`; const observationSourceId = `${sourceId}-observation-${index + 1}`;
    ...
    metrics[key] = state === "confirmed"
      ? { state, label, value, unit: fact.unit, period_start: fact.start || fact.end, period_end: fact.end, trend: "unknown", comparison_period_start: comparison?.period_start ?? null, comparison_period_end: comparison?.period_end ?? null, observations, summary: text, claim_ids: [...new Set(observations.flatMap((item) => item.claim_ids))] }
  ...
  for (const key of ["cash", "revenue", "profitability"]) {
    const selected = selectUnambiguousFact(companyFacts, definitions[key]); const fact = selected.fact;
    if (!fact) { metrics[key] = unknownMetric(key.replaceAll("_", " ")); continue; }
    addFact(key, fact, { comparable: comparableFacts(companyFacts, definitions[key]) });
  }
  const sharesTags = [["dei", "EntityCommonStockSharesOutstanding", "Common shares outstanding"]];
  const shareFacts = comparableFacts(companyFacts, sharesTags);
  let sharesOutstanding = { state: "unknown", summary: "Comparable reported shares outstanding were not established by SEC Company Facts.", observations: [], claim_ids: [] };
  ...
  if (ocf && capex && ocf.unit === capex.unit && ocf.start === capex.start && ocf.end === capex.end) {
    ...
    addFact("free_cash_flow", fcf, { summary, comparable: comparableFcf });
  ...
  if (currentDebt && noncurrentDebt && currentDebt.unit === noncurrentDebt.unit && currentDebt.end === noncurrentDebt.end) {
    ...
    addFact("debt", debt, { summary: `Total debt of ${debt.val} ${debt.unit} is calculated from current debt ${currentDebt.val} plus non-current debt ${noncurrentDebt.val} at ${debt.end}.`, comparable: comparableDebt });
  } else {
  ...
  for (const key of ["cash", "debt", "free_cash_flow", "cash_burn"]) {
    ...
    metric.state = "limited_coverage"; metric.value = null; metric.unit = null; metric.trend = "unknown"; metric.observations = [];
...
function applyExtractedFindings(fragments, findings, retrievedAt, records) {
  ...
  for (const [domain, domainFindings] of Object.entries(byDomain)) {
    ...
    domainFindings.forEach((finding, index) => {
      ...
      fragment.claims.push({ id: claimId, text: finding.statement, materiality: ["reverse_split", "going_concern", "accounting_warning", "exchange_compliance"].includes(finding.kind) ? "high" : "medium", state: finding.evidence_state, as_of: `${finding.event_date}T00:00:00Z`, source_ids: [sourceId] });
      if (!grouped.has(key)) grouped.set(key, { finding, sourceId, claimIds: [] }); grouped.get(key).claimIds.push(claimId);
      records.push(evidenceRecord({ id: `evidence-extracted-${finding.accession}-${finding.kind}-${index + 1}`, category: finding.category, state: finding.evidence_state, date: finding.event_date, text: finding.statement, sourceId, cik: fragment.identity.cik, ticker: fragment.identity.ticker }));
    });
    for (const { finding, sourceId, claimIds } of grouped.values()) fragment.sources.push({ id: sourceId, title: finding.source_title, url: finding.source_url, published_date: finding.event_date, source_type: "sec_filing", confidence: finding.confidence, retrieved_at: retrievedAt, supported_claim_ids: claimIds });
  }
  const capital = fragments.capital; const reverse = byDomain.capital.filter((item) => item.kind === "reverse_split"); const dilution = byDomain.capital.filter((item) => ["offering", "warrant", "convertible"].includes(item.kind));
  if (capital && reverse.length) capital.reverse_splits = { state: "limited_coverage", summary: "Explicit reverse-split terms were confirmed in bounded recent filings.", coverage_notes: ["Fast opens at most four selected filings and does not prove a complete five-year history."], items: reverse.map((item, index) => ({ id: `reverse-split-${index + 1}`, kind: "reverse_split", title: item.title, state: "confirmed", summary: item.statement, event_date: item.event_date, claim_ids: [item.claimId] })), claim_ids: reverse.map((item) => item.claimId) };
  if (capital && dilution.length) capital.dilution = { state: "limited_coverage", summary: "Bounded filing text identified financing instruments or capacity; only explicit completed or agreed issuances are labeled actual.", coverage_notes: ["Unopened filings and unextracted share-count terms remain outside Fast coverage."], items: dilution.map((item, index) => ({ id: `dilution-extracted-${index + 1}`, kind: item.kind, title: item.title, state: item.evidence_state, summary: item.statement, event_date: item.event_date, ...(item.value !== null && item.value !== undefined ? { value: item.value, unit: item.unit } : {}), claim_ids: [item.claimId] })), claim_ids: dilution.map((item) => item.claimId) };
  ...
  if (catalyst && compliance.length) catalyst.compliance_and_warnings = { state: "limited_coverage", summary: "Explicit exchange-compliance language was confirmed in selected filings.", coverage_notes: ["Fast does not exhaustively search exchange notices or older filings."], items: compliance.map((item, index) => ({ id: `compliance-${index + 1}`, kind: "exchange_compliance", title: item.title, state: "confirmed", summary: item.statement, event_date: item.event_date, claim_ids: [item.claimId] })), claim_ids: compliance.map((item) => item.claimId) };
  if (catalyst && events.length) { const item = events[0]; catalyst.catalyst_assessment.current = { state: "confirmed", classification: item.classification, title: item.title, event_date: item.event_date, summary: item.statement, confidence: "high", claim_ids: [item.claimId], factors: { recency: { rating: "high", explanation: "The event appears in a filing within the bounded recent window.", claim_ids: [item.claimId] }, specificity: { rating: "medium", explanation: "The filing provides a specific material-event statement.", claim_ids: [item.claimId] }, credibility: { rating: "high", explanation: "The statement comes directly from an SEC filing.", claim_ids: [item.claimId] }, novelty: factor("Novelty"), potential_significance: factor("Potential significance") } }; catalyst.catalyst_assessment.uncertainty = ["Fast does not add market context or secondary corroboration."]; }
  const financial = fragments.financial; const warnings = byDomain.financial;
  if (financial && warnings.length) {
    ...
    financial.financial_assessment.material_warnings.push(...warnings.map((item, index) => ({ id: `financial-filing-warning-${index + 1}`, kind: item.kind === "accounting_warning" ? "accounting" : "going_concern", state: "confirmed", severity: item.severity, title: item.title, as_of: item.event_date, summary: item.statement, claim_ids: [item.claimId] })));
  }
}
...
export function createSecEvidenceClient({ fetchImpl = globalThis.fetch, now = () => Date.now(), userAgent = DEFAULT_USER_AGENT, minRequestIntervalMs = 125, logger = console } = {}) {
  ...
  return {
    ...
    async researchTicker(ticker, { onProgress, budget } = {}) {
      ...
      const documents = (await Promise.all(documentPromises)).filter(Boolean); const findings = documents.flatMap(({ row, url, html }) => extractSecFilingEvidence({ html, form: row.form, filed: row.filed, accession: row.accession, documentUrl: url, documentName: row.document }));
      const exhibitCandidate = documents.find(({ row, url, html }) => CATALYST_FORMS.has(row.form) && !findings.some((finding) => finding.accession === row.accession && finding.kind === "catalyst") && findMaterialExhibitUrl(html, url));
      if (exhibitCandidate && !budget?.isStopped()) {
        ...
        if (exhibit) { cacheStatus.documents.push({ accession: exhibitCandidate.row.accession, document: exhibitName, state: exhibit.cache }); findings.push(...extractSecFilingEvidence({ html: exhibit.data, form: exhibitCandidate.row.form, filed: exhibitCandidate.row.filed, accession: exhibitCandidate.row.accession, documentUrl: exhibitUrl, documentName: exhibitName })); }
      }
      applyExtractedFindings(Object.fromEntries(Object.entries(results).map(([key, value]) => [key, value.fragment])), compactFindings(findings), retrievedAt, records);
      ...
      const packet = { ticker, identity: capitalBase.identity, retrieved_at: retrievedAt, records: structuredClone(records), sources: packetSources, cache: cacheStatus, sec_request_count: requestCounter.count }; packets.set(ticker, packet);
      const stopped = budget?.isStopped() === true;
      if (stopped || failures.length) budget?.markPartial();
      return { report: assembleFastReport(ticker, results, { generatedAt: retrievedAt }), operations: { ...operations(true), retrieval: { status: stopped || failures.length ? "limited" : "completed", sec_request_count: requestCounter.count, cache: cacheStatus, failures }, ...(budget ? { budget: budget.telemetry({ final: true }) } : {}) }, evidence_records: records, evidence_packet: packet, synthesis: { status: "pending" } };
    }
  };
}
