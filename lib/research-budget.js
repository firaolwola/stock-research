export const RESEARCH_STAGES = Object.freeze({
  fast: Object.freeze({
    stage: "fast",
    first_useful_target_ms: Object.freeze({ min: 3_000, max: 10_000 }),
    target_latency_ms: Object.freeze({ min: 3_000, max: 20_000 }),
    grace_ms: 0,
    timeout_ms: 20_000,
    ...
    difficult_cost_ceiling_usd: 0.05
  }),
  deep: Object.freeze({
    stage: "deep",
...
export function buildResearchOperations({ stage, latencyMs, firstUsefulLatencyMs = null, usage = null, webSearchCalls = 0, pricing = PRICING_SNAPSHOT, domains = null }) {
  ...
  return {
    ...
    first_useful_latency_ms: stage === "fast" && Number.isFinite(firstUsefulLatencyMs) ? Math.max(0, Math.round(firstUsefulLatencyMs)) : null,
    within_first_useful_target: stage === "fast" && Number.isFinite(firstUsefulLatencyMs) ? firstUsefulLatencyMs <= budget.first_useful_target_ms.max : null,
    domains,
