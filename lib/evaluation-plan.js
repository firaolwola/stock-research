  ...
  const visit = async (reference) => {
    ...
    if (parentReference !== null) {
      ...
      if (expectedHash !== null && expectedHash !== undefined) {
        ...
        const canonicalParentBytes = parent.bytes.toString().replace(/\r\n/g, "\n");
        if (createHash("sha256").update(canonicalParentBytes).digest("hex") !== expectedHash.toLowerCase()) throw new EvaluationPlanError("evaluation_plan_parent_hash_mismatch");
      }
