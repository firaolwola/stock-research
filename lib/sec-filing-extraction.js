...
function splitEventContexts(text, limit = 24) {
  ...
  return matches.map((match, index) => {
    ...
    const rawStart = Math.max(leftBoundary, match.index - 320); const rawEnd = Math.min(rightBoundary, match.index + match[0].length + 260);
...
export function extractSecFilingEvidenceWithDiagnostics({ html, form, filed, reportDate = null, accession, documentUrl, documentName, evaluatedAt }) {
  ...
  if (/^NT (?:10-K|20-F|40-F)$/.test(form)) {
    const lateStatement = nearbySentence(all, /\b(?:annual report|form (?:10-K|20-F|40-F))\b.{0,300}\b(?:could not|cannot|unable to|will not)\b.{0,180}\b(?:filed|file)\b.{0,180}\b(?:prescribed|required|due)\b/i)
      ?? `The issuer filed ${form} for the annual period ended ${reportDate ?? "an unresolved period"}; the bounded filing text did not yield a filing-specific delay reason.`;
    add("late_annual_filing", "going_concern_accounting", "Delayed annual filing", lateStatement, { severity: "high", expected_period: reportDate, delay_reason_extracted: !/did not yield/.test(lateStatement) });
  }
  ...
  const item402NonReliance = form === "8-K" && item402 && /\b(?:previously issued financial statements?.{0,300}should no longer be relied upon|concluded|determined|accounting error)\b/i.test(item402) ? item402 : null;
