...
function splitEventContexts(text, limit = 24) {
  ...
  return matches.map((match, index) => {
    ...
    const rightBoundary = next ? next.index : text.length;
    const start = Math.max(leftBoundary, match.index - 220); const end = Math.min(rightBoundary, match.index + match[0].length + 260);
    return { text: text.slice(start, end).trim(), anchor: match.index - start, index: match.index, span_id: `split-span-${index + 1}` };
  });
}
...
export function extractSecFilingEvidenceWithDiagnostics({ html, form, filed, accession, documentUrl, documentName, evaluatedAt }) {
  ...
  for (const reverseContext of reverseContexts) {
    ...
    if (!reverseRatio) { corporateActionDiagnostics.push({ source_accession: accession, filing_form: form, local_text_span_id: reverseContext.span_id, extracted_ratio: null, extracted_status: "unknown", extracted_effective_date: null, source_filing_date: filed, canonical_event_id: null, disposition: "rejected", reason: "ratio_not_established" }); priorMention = { ...reverseContext, ratio: null }; continue; }
    if (authorizationRange(reverse)) {
      corporateActionDiagnostics.push({ source_accession: accession, filing_form: form, local_text_span_id: reverseContext.span_id, extracted_ratio: reverseRatio.label, extracted_status: "authorized", extracted_effective_date: null, source_filing_date: filed, canonical_event_id: null, disposition: "withheld", reason: "authorization_range_is_not_a_completed_action" });
    ...
    corporateActionDiagnostics.push({ source_accession: accession, filing_form: form, local_text_span_id: reverseContext.span_id, extracted_ratio: reverseRatio.label, extracted_status: action.state, extracted_effective_date: eventDate, source_filing_date: filed, canonical_event_id: null, disposition: "accepted", reason: "raw_occurrence_accepted_for_canonicalization" });
