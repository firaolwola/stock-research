...
export function extractFilingCapitalExpenditureFacts({ html, form, filed, reportDate = null, accession, documentUrl, documentName, currencyHint = null }) {
  const facts = []; const diagnostics = []; const tables = [...String(html ?? "").matchAll(/<table\b[^>]*>[\s\S]*?<\/table>/gi)].slice(0, 24);
  ...
  tables.forEach((tableMatch, tableIndex) => {
    ...
    for (const { row, rowIndex, labelIndex } of capexRows) {
      ...
      if (!reason && consolidatedIndex >= 0 && periods.length === 1 && rawValueCells.length !== periods.length && rawValueCells.length > 0) {
