...
function splitEventContexts(text, limit = 24) {
  ...
  return matches.map((match, index) => {
    ...
    // Midpoints prevent an adjacent action's date from being borrowed.
    const previous = matches[index - 1]; const next = matches[index + 1];
    const leftBoundary = previous ? Math.floor((previous.index + previous[0].length + match.index) / 2) : 0;
    const rightBoundary = next ? Math.floor((match.index + match[0].length + next.index) / 2) : text.length;
    const start = Math.max(leftBoundary, match.index - 220); const end = Math.min(rightBoundary, match.index + match[0].length + 260);
    return { text: text.slice(start, end).trim(), anchor: match.index - start, index: match.index };
  });
}
...
function effectiveDate(text, anchor = null) {
  const pattern = /\b(?:effective|effected|implemented|completed|became effective)(?:\s+(?:on|as of|at[^,.]{0,80}on))?\s+((?:January|February|March|April|May|June|July|August|September|October|November|December)\s+\d{1,2},\s+\d{4}|\d{4}-\d{2}-\d{2})\b/gi;
  const matches = [...String(text ?? "").matchAll(pattern)]; const match = anchor === null ? matches[0] : matches.sort((a, b) => Math.abs(a.index - anchor) - Math.abs(b.index - anchor))[0];
...
function resolvedSplitState(context, filed, evaluatedAt, anchor = null) {
...
export function extractSecFilingEvidence({ html, form, filed, accession, documentUrl, documentName, evaluatedAt }) {
  ...
  for (const reverseContext of reverseContexts) {
    ...
    if (!reverseRatio && priorMention?.ratio && reverseContext.index - priorMention.index <= 320 && !effectiveDate(priorMention.text, priorMention.anchor) && effectiveDate(reverse, reverseContext.anchor)) reverseRatio = priorMention.ratio;
    if (!reverseRatio) { priorMention = { ...reverseContext, ratio: null }; continue; }
    const action = resolvedSplitState(reverse, filed, evaluatedAt, reverseContext.anchor);
    ...
    add("reverse_split", "reverse_splits", `${label} ${reverseRatio.label} reverse split`, reverse, { event_date: eventDate, announced_date: announced, effective_date: action.date, completed_date: completed, ratio: reverseRatio.label, split_factor: reverseRatio.factor, action_state: action.state });
  ...
  return findings.filter((item) => !(item.kind === "reverse_split" && !item.event_date && datedSplitRatios.has(item.ratio))).slice(0, 12);
}
