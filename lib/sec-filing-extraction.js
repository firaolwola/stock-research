...
function splitActionState(sentence) {
  if (/\b(?:filed a certificate of amendment.{0,300}(?:effect|effectuate)|became effective|was effective|effected|implemented|completed|distributed|split-adjusted trading (?:began|commenced))\b/i.test(sentence)) return "completed";
...
export function extractSecFilingEvidence({ html, form, filed, accession, documentUrl, documentName }) {
  ...
  const reverse = contextAround(text, /\breverse (?:stock )?split\b/i); const reverseRatio = splitRatio(reverse);
  ...
  if (forward && forwardRatio && !/\breverse (?:stock )?split\b/i.test(forward)) {
    const actionState = splitActionState(forward);
    add("stock_split", "financial_context", `${actionState === "completed" ? "Completed" : actionState === "authorized" ? "Authorized" : actionState === "proposed" ? "Proposed" : "Unresolved"} ${forwardRatio.label} stock split`, forward, { ratio: forwardRatio.label, split_factor: forwardRatio.factor, action_state: actionState });
  }
  ...
  const accounting = contextAround(text, /\b(?:material weakness(?:es)?|internal control(?:s)? over financial reporting.{0,300}(?:not effective|ineffective)|(?:not effective|ineffective).{0,300}internal control(?:s)? over financial reporting|disclosure controls? and procedures? (?:were|was) not effective)\b/i, 200, 800);
  ...
  const compliance = contextAround(text, /\b(?:Nasdaq|NYSE|exchange).{0,300}\b(?:noncompliance|non-compliance|deficien(?:cy|t)|delist(?:ing|ed)|continued listing standards?|minimum bid price|regained compliance|now complies)\b|\b(?:noncompliance|deficien(?:cy|t)|regained compliance|now complies).{0,300}\b(?:Nasdaq|NYSE|exchange)\b/i, 100, 700);
  if (compliance) {
  ...
  if (["8-K", "6-K"].includes(form)) {
    ...
    if (catalyst) {
      const classification = nonReliance ? "accounting" : /offering|purchase agreement|issued and sold|convertible|warrant/i.test(catalyst) ? "financing" : /acqui|merger|business combination/i.test(catalyst) ? "corporate_action" : /appoint|resign|management/i.test(catalyst) ? "management" : /approval|clearance/i.test(catalyst) ? "regulatory" : /contract|award/i.test(catalyst) ? "contract" : "other";
