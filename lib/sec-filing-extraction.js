...
// SEC disclosures also spell out ratios as “one share ... for every ten
// shares”. Keep the intervening security description bounded and sentence
// local so unrelated share counts cannot be paired.
const VERBOSE_RATIO_PATTERN = new RegExp(`(?:ratio of\\s+)?([0-9][0-9,]*|${NUMBER_WORD})\\s+shares?\\s+of\\s+[^.;]{0,120}?\\s+for\\s+every\\s+([0-9][0-9,]*|${NUMBER_WORD})\\s+shares?(?![a-z0-9-])`, "gi");
