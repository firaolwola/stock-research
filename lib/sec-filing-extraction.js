...
function effectiveDate(text, anchor = null) {
  ...
  const effectiveDateOf = [...textValue.matchAll(new RegExp(`\\b(?:effective date|date of effectiveness)\\s+(?:of|for)?\\s*(${DATE_TOKEN_SOURCE})\\b`, "gi"))].map((match) => ({ match, actionAnchor: match.index }));
...
function splitDateRole(text, selectedDate, actionState, statedLifecycle, historicalList) {
  ...
  const rolePatterns = {
    ...
    effective_date: [
      ...
      new RegExp(`\\b(?:effective date|date of effectiveness)\\s+(?:of|for)?\\s*(${DATE_TOKEN_SOURCE})\\b`, "gi")
    ],
    authorization_date: [
...
export function extractSecFilingEvidenceWithDiagnostics({ html, form, filed, reportDate = null, accession, documentUrl, documentName, evaluatedAt }) {
  ...
  for (const reverseContext of reverseContexts) {
    ...
    if (action.state === "unknown" && classifiedDate.role === "effective_date" && eventDate
