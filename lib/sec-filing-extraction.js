...
const CAPITAL_EXPENDITURE_LABEL = /\b(?:capital expenditures?|capital additions|payments? (?:to acquire|for) property(?:,? plant(?:,? and)? equipment| and equipment)?|purchases? of property(?:,? plant(?:,? and)? equipment| and equipment)?|additions to property(?:,? plant(?:,? and)? equipment| and equipment)?)\b/i;
...
export function extractFilingCapitalExpenditureFacts({ html, form, filed, reportDate = null, accession, documentUrl, documentName, currencyHint = null }) {
  ...
  tables.forEach((tableMatch, tableIndex) => {
    ...
    const row = rows.find((candidate) => candidate.cells.some((cell) => CAPITAL_EXPENDITURE_LABEL.test(cell)));
    const labelIndex = row?.cells.findIndex((cell) => CAPITAL_EXPENDITURE_LABEL.test(cell)) ?? -1;
    const consolidatedIndex = consolidatedColumnIndex(rows);
    const rawValueCells = labelIndex < 0 ? [] : row.cells.slice(labelIndex + 1);
    let values = rawValueCells.map((cell) => financialNumber(cell, scale)).filter((value) => value !== null);
    let columnSelection = "period_columns";
    let reason = !currency ? "currency_not_explicit" : !periods.length ? "comparable_periods_not_explicit" : !row ? "capex_row_not_isolated" : null;
    // Some 10-Ks present one annual period as separate segment tables. When an
    // explicit Consolidated column is present, select only that value rather
    // than treating the segment values as multiple periods. Keep this bounded
    // to one-period tables so a more complex layout cannot be guessed.
    ...
    diagnostics.push({ accession, form, document: documentName, table_index: tableIndex, currency, currency_source: currencySource, scale, periods_detected: periods.length, values_detected: values.length, column_selection: columnSelection, consolidated_column_index: consolidatedIndex >= 0 ? consolidatedIndex : null, disposition: reason ? "withheld" : "accepted", reason });
    if (reason) return;
    periods.forEach((period, index) => facts.push({ val: values[index], start: period.start, end: period.end, filed, accn: accession, form, unit: currency, taxonomy: "sec-filing", tag: "CapitalExpenditures", label: "Capital expenditures", source_type: "sec_filing_table", source_url: documentUrl, source_title: `${form} filed ${filed} — ${documentName} (capital-expenditure table)`, source_id: `source-sec-filing-capex-${accession.replaceAll("-", "")}-${tableIndex + 1}-${period.end}` }));
  });
