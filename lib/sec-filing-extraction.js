...
const CAPITAL_EXPENDITURE_LABEL = /\b(?:capital expenditures?|capital additions|payments? (?:to acquire|for) (?:property(?:,? plant(?:,? and)? equipment| and equipment)?|patents?|trademarks?|intangible assets?)|purchases? of (?:property(?:,? plant(?:,? and)? equipment| and equipment)?|patents?|trademarks?|intangible assets?)|additions to (?:property(?:,? plant(?:,? and)? equipment| and equipment)?|intangible assets?))\b/i;
const FINANCIAL_DATE = /\b(?:January|February|March|April|May|June|July|August|September|October|November|December)\s+\d{1,2}(?:,\s+20\d{2})?\b/i;
const FINANCIAL_CURRENCY = /\b(USD|CAD|CNY|EUR|GBP|AUD|JPY|CHF)\b/i;
const FILING_TABLE_FORMS = new Set(["10-K", "10-Q", "20-F", "40-F"]);

function tableRows(html) {
  return [...String(html ?? "").matchAll(/<tr\b[^>]*>([\s\S]*?)<\/tr>/gi)].map((rowMatch) => {
    const cells = [...rowMatch[1].matchAll(/<(?:td|th)\b[^>]*>([\s\S]*?)<\/(?:td|th)>/gi)].map((cell) => filingHtmlToText(cell[1]));
    return { cells, text: cells.join(" "), html: rowMatch[1] };
  }).filter((row) => row.cells.length);
}

function financialScale(text) {
  const match = /(?:\b(?:in|amounts? in|USD|CAD|CNY|EUR|GBP|AUD|JPY|CHF)|US\$|\$)\s*(?:in\s*)?(?:\(|:)?\s*(thousands?|millions?|billions?)\b/i.exec(String(text ?? ""));
  if (!match) return 1;
  return { thousand: 1_000, million: 1_000_000, billion: 1_000_000_000 }[match[1].toLowerCase().replace(/s$/, "")] ?? 1;
}

function financialCurrency(text) {
  const code = FINANCIAL_CURRENCY.exec(String(text ?? ""))?.[1]?.toUpperCase();
  if (code) return code;
  return /(?:\$|US\s*dollars?)/i.test(String(text ?? "")) ? "USD" : null;
}

function financialNumber(cell, scale) {
  const value = String(cell ?? "").replace(/[\u2212−]/g, "-").replace(/\s+/g, " ").trim();
  if (!value || /^(?:-|—|–|n\/a|na)$/i.test(value)) return null;
  const match = /\(?\s*(-?\d[\d,]*(?:\.\d+)?)\s*\)?/.exec(value);
  if (!match || /\d{4}-\d{2}-\d{2}/.test(value)) return null;
  const number = Number(match[1].replaceAll(",", ""));
  if (!Number.isFinite(number)) return null;
  return (value.includes("(") && !match[1].startsWith("-")) ? -number * scale : number * scale;
}

function financialPeriodStart(end, cadence) {
  const parsed = new Date(`${end}T00:00:00Z`);
  if (Number.isNaN(parsed.getTime())) return null;
  if (cadence === "annual") return `${parsed.getUTCFullYear()}-01-01`;
  if (cadence === "year_to_date") return `${parsed.getUTCFullYear()}-01-01`;
  if (cadence === "quarter") {
    const month = parsed.getUTCMonth(); const year = parsed.getUTCFullYear();
    return `${new Date(Date.UTC(year, month - 3, 1)).toISOString().slice(0, 10)}`;
  }
  return null;
}

function filingPeriods(rows, form, reportDate) {
  const header = rows.find((row) => /\b(?:years?|three|six|nine|twelve)\s+months?|quarter(?:s)?\s+ended|as of\b/i.test(row.text) && /\b20\d{2}\b/.test(row.text))
    ?? rows.find((row) => FINANCIAL_DATE.test(row.text) && /\b20\d{2}\b/.test(row.text))
    ?? rows.find((row) => row.cells.filter((cell) => /\b20\d{2}\b/.test(cell)).length >= 2);
  const headerText = header?.text ?? "";
  const date = FINANCIAL_DATE.exec(headerText)?.[0];
  const monthDay = date ? date.replace(/,\s+20\d{2}/, "") : null;
  const years = [...headerText.matchAll(/\b(20\d{2})\b/g)].map((match) => Number(match[1]));
  const uniqueYears = [...new Set(years)];
  const annual = /\byears?\s+ended\b/i.test(headerText)
    || (uniqueYears.length >= 2 && /^(?:10-K|20-F|40-F)$/.test(form))
    || (uniqueYears.length === 1 && FINANCIAL_DATE.test(headerText) && /^(?:10-K|20-F|40-F)$/.test(form));
  const quarter = /\b(?:three|quarter)\s+months?|\bthree\s+months?\s+ended\b/i.test(headerText);
  const ytd = /\b(?:six|nine|twelve)\s+months?\s+ended\b/i.test(headerText);
  const cadence = quarter ? "quarter" : ytd ? "year_to_date" : annual ? "annual" : null;
  if (!cadence) {
    if (reportDate && /^(?:10-K|20-F|40-F)$/.test(form)) return [{ start: `${reportDate.slice(0, 4)}-01-01`, end: reportDate, cadence: "annual" }];
    return [];
  }
  return uniqueYears.map((year) => {
    const parsedEnd = monthDay ? new Date(`${monthDay}, ${year} 00:00:00 UTC`) : null;
    const end = parsedEnd && !Number.isNaN(parsedEnd.getTime()) ? parsedEnd.toISOString().slice(0, 10) : `${year}-12-31`;
    return { start: financialPeriodStart(end, cadence), end, cadence };
  }).filter((period) => period.start && period.end);
}

function capitalExpenditureType(label) {
  const text = String(label ?? "");
  if (/\b(?:patents?|trademarks?|intangible assets?)\b/i.test(text)) return "intangible_assets";
  if (/\b(?:property|plant|equipment|ppe|capital additions)\b/i.test(text)) return "property_plant_equipment";
  return "total_capital_expenditure";
}

function consolidatedColumnIndex(rows) {
  for (const row of rows) {
    const index = row.cells.findIndex((cell) => /^\s*consolidated\s*$/i.test(cell));
...
/**
 * Extract only explicit capital-expenditure rows from a small set of SEC filing
 * tables. This intentionally returns no facts when currency, period cadence,
 * or column alignment is ambiguous; callers must retain the Limited/Unscored
 * state rather than infer FCF from an OCF-only value.
 */
export function extractFilingCapitalExpenditureFacts({ html, form, filed, reportDate = null, accession, documentUrl, documentName, currencyHint = null }) {
  const facts = []; const diagnostics = [];
  // SEC inline-XBRL filings can contain hundreds of layout tables before the
  // actual financial table. Select only tables whose bounded text contains a
  // capital-expenditure label, then inspect a small deterministic candidate
  // set. Preserve the original table index for diagnostics.
  const tables = [...String(html ?? "").matchAll(/<table\b[^>]*>[\s\S]*?<\/table>/gi)]
    .map((match, index) => ({ match, index }))
    .filter(({ match }) => CAPITAL_EXPENDITURE_LABEL.test(filingHtmlToText(match[0])))
    .slice(0, 24);
  if (!FILING_TABLE_FORMS.has(String(form ?? "").toUpperCase())) return { facts, diagnostics: [{ accession, form, document: documentName, table_index: null, currency: null, scale: null, periods_detected: 0, values_detected: 0, disposition: "withheld", reason: "form_not_supported" }] };
  if (!String(accession ?? "").trim()) return { facts, diagnostics: [{ accession: null, form, document: documentName, table_index: null, currency: null, scale: null, periods_detected: 0, values_detected: 0, disposition: "withheld", reason: "accession_missing" }] };
  tables.forEach(({ match: tableMatch, index: tableIndex }) => {
    const tableHtml = tableMatch[0]; const rows = tableRows(tableHtml); const tableText = filingHtmlToText(tableHtml);
    if (!CAPITAL_EXPENDITURE_LABEL.test(tableText)) return;
    // SEC filing tables frequently put the unit/currency note immediately
    // before or after the table instead of in its caption. Keep this fallback
    // tightly bounded to neighboring markup so another table cannot donate
    // units or values to the capex row.
    const rawHtml = String(html ?? "");
    const before = rawHtml.slice(Math.max(0, tableMatch.index - 600), tableMatch.index);
    const afterStart = tableMatch.index + tableHtml.length;
    const after = rawHtml.slice(afterStart, Math.min(rawHtml.length, afterStart + 600));
    const previousTableEnd = before.toLowerCase().lastIndexOf("</table>");
    const adjacentBefore = previousTableEnd >= 0 ? before.slice(previousTableEnd + "</table>".length) : before;
    const nextTable = after.toLowerCase().indexOf("<table");
    const adjacentAfter = nextTable >= 0 ? after.slice(0, nextTable) : after;
    const boundedContext = filingHtmlToText(`${adjacentBefore} ${adjacentAfter}`);
    const detectedCurrency = financialCurrency(`${tableText} ${boundedContext}`);
    const currency = detectedCurrency ?? (/^[A-Z]{3}$/.test(String(currencyHint ?? "")) ? String(currencyHint).toUpperCase() : null);
    const currencySource = detectedCurrency ? "table_context" : currency ? "caller_hint" : null;
    const scale = financialScale(`${tableText} ${boundedContext}`); const periods = filingPeriods(rows, form, reportDate);
    const capexRows = rows.map((candidate, rowIndex) => ({ row: candidate, rowIndex, labelIndex: candidate.cells.findIndex((cell) => CAPITAL_EXPENDITURE_LABEL.test(cell)) })).filter(({ labelIndex }) => labelIndex >= 0);
    const consolidatedIndex = consolidatedColumnIndex(rows);
    if (!capexRows.length) {
      diagnostics.push({ accession, form, document: documentName, table_index: tableIndex, row_index: null, currency, currency_source: currencySource, scale, periods_detected: periods.length, values_detected: 0, column_selection: "period_columns", consolidated_column_index: consolidatedIndex >= 0 ? consolidatedIndex : null, disposition: "withheld", reason: !currency ? "currency_not_explicit" : !periods.length ? "comparable_periods_not_explicit" : "capex_row_not_isolated" });
      return;
    }
    for (const { row, rowIndex, labelIndex } of capexRows) {
      const rawValueCells = row.cells.slice(labelIndex + 1);
      let values = rawValueCells.map((cell) => financialNumber(cell, scale)).filter((value) => value !== null);
      let columnSelection = "period_columns";
      // A non-cash acquisition or capitalization disclosure is not a cash
      // outflow and must never be used as CapEx for FCF. Only reject the row
      // when the row label itself says non-cash; a surrounding table heading
      // may legitimately describe other non-cash items in the same table.
      let reason = /\bnon[-\s]?cash\b/i.test(row.cells[labelIndex])
        ? "non_cash_capex"
        : !currency ? "currency_not_explicit" : !periods.length ? "comparable_periods_not_explicit" : null;
      const segmentedHeaders = /U\.?\s*S\.?\s+Markets[\s\S]*International Markets[\s\S]*Consolidated/i.test(tableText);
      // Inline-XBRL table headers often use colspan while spacer cells in the
      // data row preserve the underlying grid. In that shape the header's
      // index points at an intermediate segment value; explicit segment labels
      // make the final numeric cell the authoritative Consolidated value.
      const expandedSegmentGrid = row.cells.length > 6;
      if (!reason && periods.length === 1 && segmentedHeaders && expandedSegmentGrid && values.length >= 3) {
        values = [values.at(-1)]; columnSelection = "explicit_consolidated_segmented";
      }
      // Some 10-Ks present one annual period as separate segment tables. When
      // an explicit Consolidated column is present, select only that value
      // rather than treating segment values as multiple periods. Keep this
      // bounded to one-period tables so a complex layout cannot be guessed.
      if (!reason && columnSelection === "period_columns" && consolidatedIndex >= 0 && periods.length === 1 && rawValueCells.length !== periods.length && rawValueCells.length > 0) {
        if (consolidatedIndex <= labelIndex || consolidatedIndex >= row.cells.length) reason = "consolidated_column_unavailable";
        else {
          const consolidatedValue = financialNumber(row.cells[consolidatedIndex], scale);
          if (consolidatedValue === null) reason = "consolidated_value_missing";
          else { values = [consolidatedValue]; columnSelection = "explicit_consolidated"; }
        }
      }
      // A bounded set of SEC 10-K tables is rendered as a flattened text cell
      // rather than distinct HTML columns. Accept that shape only when the
      // table explicitly names all three segment columns and the capex label
      // is followed by at least three numeric values; the last value is then
      // the stated Consolidated amount. Never apply this fallback to a generic
      // prose match or an ambiguous number of segment values.
      if (!reason && periods.length === 1 && !values.length && rawValueCells.length === 0
        && /U\.?\s*S\.?\s+Markets[\s\S]*International Markets[\s\S]*Consolidated/i.test(tableText)) {
        // A live SEC rendering can split the capex label and its values into
        // separate rows/cells. Prefer the row text, but fall back to the
        // bounded table text so those values are still bound to this table.
        const sourceText = row.text.match(/\d/) ? row.text : tableText;
        const flattenedLabelMatch = CAPITAL_EXPENDITURE_LABEL.exec(sourceText);
        const afterLabel = flattenedLabelMatch ? sourceText.slice(flattenedLabelMatch.index + flattenedLabelMatch[0].length) : "";
        const flattenedValues = [...afterLabel.matchAll(/\(?\s*-?\d[\d,]*(?:\.\d+)?\s*\)?/g)].map((match) => financialNumber(match[0], scale)).filter((value) => value !== null);
        if (flattenedValues.length >= 3) {
          values = [flattenedValues.at(-1)]; columnSelection = "flattened_consolidated";
        }
      }
      if (!reason && periods.length === 1 && !values.length && rawValueCells.length === 0
        && /\bConsolidated\b[\s\S]{0,160}Capital expenditures/i.test(tableText)) {
        const flattenedLabelMatch = CAPITAL_EXPENDITURE_LABEL.exec(row.text);
        const afterLabel = flattenedLabelMatch ? row.text.slice(flattenedLabelMatch.index + flattenedLabelMatch[0].length) : "";
        const flattenedValues = [...afterLabel.matchAll(/\(?\s*-?\d[\d,]*(?:\.\d+)?\s*\)?/g)].map((match) => financialNumber(match[0], scale)).filter((value) => value !== null);
        // A single explicit Consolidated value is safe; multiple values
        // without segment columns remain ambiguous and are withheld below.
        if (flattenedValues.length === 1) {
          values = flattenedValues; columnSelection = "flattened_consolidated_single";
        }
      }
      if (!reason && values.length !== periods.length) reason = "period_value_column_mismatch";
      diagnostics.push({ accession, form, document: documentName, table_index: tableIndex, row_index: rowIndex, currency, currency_source: currencySource, scale, periods_detected: periods.length, values_detected: values.length, column_selection: columnSelection, consolidated_column_index: consolidatedIndex >= 0 ? consolidatedIndex : null, disposition: reason ? "withheld" : "accepted", reason });
      if (reason) continue;
      const expenditureType = capitalExpenditureType(row.cells[labelIndex]);
      periods.forEach((period, index) => facts.push({ val: values[index], start: period.start, end: period.end, filed, accn: accession, form, unit: currency, taxonomy: "sec-filing", tag: "CapitalExpenditures", label: "Capital expenditures", capital_expenditure_type: expenditureType, capital_expenditure_component: row.cells[labelIndex].trim().toLowerCase(), source_type: "sec_filing_table", source_url: documentUrl, source_title: `${form} filed ${filed} — ${documentName} (capital-expenditure table)`, source_id: `source-sec-filing-capex-${accession.replaceAll("-", "")}-${tableIndex + 1}-row${rowIndex + 1}-${period.end}` }));
    }
  });
...
// SEC disclosures also spell out ratios as “ratio of one share ... for every
// ten shares”. Require the explicit ratio cue: without it, an authorized-share
// count earlier in the sentence can be paired with the later “for every”
// denominator and become a fabricated split (for example,
// “550,000,000 shares ... reverse stock split at a ratio of one share ...”).
// The intervening security description remains bounded and sentence-local.
const VERBOSE_RATIO_PATTERN = new RegExp(`\\bratio of\\s+([0-9][0-9,]*|${NUMBER_WORD})\\s+shares?\\s+of\\s+[^.;]{0,120}?\\s+for\\s+every\\s+([0-9][0-9,]*|${NUMBER_WORD})\\s+shares?(?![a-z0-9-])`, "gi");
...
function ratioMatches(sentence) {
  const compact = [...String(sentence ?? "").matchAll(new RegExp(RATIO_PATTERN.source, RATIO_PATTERN.flags))].map((match) => ({
    match,
    start: match.index,
  ...
  const verbose = [...String(sentence ?? "").matchAll(new RegExp(VERBOSE_RATIO_PATTERN.source, VERBOSE_RATIO_PATTERN.flags))].map((match) => ({
    match,
    start: match.index,
    end: match.index + match[0].length,
    text: match[0],
  ...
  return [...compact, ...verbose].sort((left, right) => left.start - right.start);
}
...
function effectiveDate(text, anchor = null) {
  ...
  const effectiveDateOf = [...textValue.matchAll(new RegExp(`\\b(?:effective date|date of effectiveness)\\b\\s*(?:(?:(?:of|for)(?:\\s+(?:the\\s+)?(?:action|split|which))?|thereof)\\s*)?(?:was|is|being)?\\s*(?::\\s*)?(${DATE_TOKEN_SOURCE})\\b`, "gi"))].map((match) => ({ match, actionAnchor: match.index }));
  // Filings may put the calendar date first, then identify it as the
  // effective date of the reverse split (for example, "August 24, 2023 was
  // the effective date of the one-for-ten reverse stock split"). Keep this
  // bounded to an adjacent split reference so unrelated dates cannot bind.
  const dateBeforeEffective = [...textValue.matchAll(new RegExp(`\\b(${DATE_TOKEN_SOURCE})\\b.{0,140}\\b(?:was|is|became)\\s+(?:the\\s+)?(?:effective date|date of effectiveness)\\b.{0,120}\\b(?:reverse (?:stock )?split|split)\\b`, "gi"))].map((match) => ({ match, actionAnchor: match.index }));
  ...
  const selected = anchor === null ? [...after, ...before, ...trading, ...effectiveDateOf, ...dateBeforeEffective, ...generic][0] : [...after, ...before, ...preceding, ...trading, ...effectiveDateOf, ...dateBeforeEffective, ...generic].sort((a, b) => Math.abs(a.actionAnchor - anchor) - Math.abs(b.actionAnchor - anchor))[0];
...
function splitDateRole(text, selectedDate, actionState, statedLifecycle, historicalList) {
  ...
  const rolePatterns = {
    ...
    effective_date: [
      ...
      new RegExp(`\\b(?:effective date|date of effectiveness)\\b\\s*(?:(?:(?:of|for)(?:\\s+(?:the\\s+)?(?:action|split|which))?|thereof)\\s*)?(?:was|is|being)?\\s*(?::\\s*)?(${DATE_TOKEN_SOURCE})\\b`, "gi"),
      new RegExp(`\\b(${DATE_TOKEN_SOURCE})\\b.{0,140}\\b(?:was|is|became)\\s+(?:the\\s+)?(?:effective date|date of effectiveness)\\b.{0,120}\\b(?:reverse (?:stock )?split|split)\\b`, "gi")
    ],
    authorization_date: [
