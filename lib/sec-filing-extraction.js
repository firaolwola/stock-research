...
function filingPeriods(rows, form, reportDate) {
  ...
  const annual = /\byears?\s+ended\b/i.test(headerText) || (uniqueYears.length >= 2 && /^(?:10-K|20-F|40-F)$/.test(form));
 ...
 */
export function extractFilingCapitalExpenditureFacts({ html, form, filed, reportDate = null, accession, documentUrl, documentName }) {
  ...
  tables.forEach((tableMatch, tableIndex) => {
    ...
    const currency = financialCurrency(`${tableText} ${boundedContext}`); const scale = financialScale(`${tableText} ${boundedContext}`); const periods = filingPeriods(rows, form, reportDate);
    ...
    const values = labelIndex < 0 ? [] : row.cells.slice(labelIndex + 1).map((cell) => financialNumber(cell, scale)).filter((value) => value !== null);
    const reason = !currency ? "currency_not_explicit" : !periods.length ? "comparable_periods_not_explicit" : !row ? "capex_row_not_isolated" : values.length !== periods.length ? "period_value_column_mismatch" : null;
    diagnostics.push({ accession, form, document: documentName, table_index: tableIndex, currency, scale, periods_detected: periods.length, values_detected: values.length, disposition: reason ? "withheld" : "accepted", reason });
    if (reason) return;
