...
function splitEventContexts(text, limit = 24) {
  ...
  return matches.map((match) => {
    const start = Math.max(0, match.index - 240); const end = Math.min(text.length, match.index + match[0].length + 600);
    return { text: text.slice(start, end).trim(), anchor: match.index - start };
  });
}
...
export function extractSecFilingEvidence({ html, form, filed, accession, documentUrl, documentName, evaluatedAt }) {
  ...
  for (const reverseContext of reverseContexts) {
    const reverse = reverseContext.text; const reverseRatio = splitRatio(reverse, reverseContext.anchor); if (!reverseRatio) continue;
