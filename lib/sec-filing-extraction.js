...
function splitEventContexts(text, limit = 24) {
  ...
  return matches.map((match, index) => {
    ...
    const start = Math.max(leftBoundary, match.index - 320); const end = Math.min(rightBoundary, match.index + match[0].length + 260);
    ...
    return { text: segment, anchor: match.index - start, index: match.index, start, end, left_boundary: leftBoundary, right_boundary: rightBoundary, source_length: text.length, span_truncated: start > leftBoundary || end < rightBoundary, span_id: `split-span-${index + 1}`, segment_id: `split-segment-${index + 1}`, historical_list: historicalList };
  });
}
...
function effectiveDate(text, anchor = null) {
  ...
  const selected = anchor === null ? [...after, ...before, ...trading, ...generic][0] : [...after, ...before, ...preceding, ...trading, ...generic].sort((a, b) => Math.abs(a.actionAnchor - anchor) - Math.abs(b.actionAnchor - anchor))[0];
...
function localSplitDate(text, anchor, ratioPosition = anchor) {
  ...
  const before = matches.filter((match) => {
    ...
    const bindingClause = String(text).slice(match.index + match[0].length, ratioPosition);
    return !/[.;]|\band\b/i.test(bindingClause) && /\b(?:effected|effectuated|implemented|completed)\b/i.test(bindingClause);
...
function splitDateRole(text, selectedDate, actionState, statedLifecycle, historicalList) {
  ...
  const rolePatterns = {
    ...
    effective_date: [
      ...
      new RegExp(`\\b(?:on|as of)\\s+(${DATE_TOKEN_SOURCE})\\b.{0,140}\\b(?:effected|effectuated|implemented)\\b`, "gi")
    ],
    authorization_date: [
...
function lifecyclePosition(text, state) {
  const patterns = {
    completed: /\b(?:became effective|was effective|effected|effectuated|implemented|completed|distributed)\b/i,
...
export function extractSecFilingEvidenceWithDiagnostics({ html, form, filed, accession, documentUrl, documentName, evaluatedAt }) {
  ...
  for (const reverseContext of reverseContexts) {
    ...
    const ratios = ratioMatches(reverse); const competingRatio = ratios.length > 1;
    ...
    const diagnosticBase = { source_accession: accession, filing_form: form, local_text_span_id: reverseContext.span_id, segment_id: reverseContext.segment_id, candidate_date_source_segment: null, candidate_lifecycle_source_segment: null, inheritance_attempted: false, inheritance_rejection_reason: null, competing_ratio_detected: competingRatio, competing_ratio_positions: ratios.map((item) => item.start), source_span_truncated: reverseContext.span_truncated, ratio_token_touched_truncation_boundary: ratioTouchedTruncationBoundary, complete_ratio_token_text: reverseRatio?.text ?? null, ratio_position: reverseRatio?.start ?? null, date_position: null, extracted_date: null, date_role: "unknown_date_role", date_role_evidence: "date_not_yet_classified", canonical_chosen_event_date: null, merge_target: null, merge_reason: null, filing_vs_effective_reconciliation: "not_attempted", canonical_acceptance_invariant_passed: false, canonical_validation_reason: null, issuer_identity_match: true, source_filing_date: filed, canonical_event_id: null };
    ...
    const classifiedDate = splitDateRole(reverse, eventDate, action.state, statedLifecycle, reverseContext.historical_list);
    const selectedDatePosition = datePosition(reverse, eventDate); const selectedLifecyclePosition = action.lifecycle_source === "authoritative_retrospective_history" ? reverseContext.anchor : lifecyclePosition(reverse, statedLifecycle === "scheduled" && action.state === "completed" ? "scheduled" : action.state);
    ...
    const interveningCompetingRatio = ratios.some((item) => item.start !== reverseRatio.start && item.start > bindingStart && item.start < bindingEnd);
  ...
  return { findings: [...primaryFindings, ...filingSupport], corporate_action_diagnostics: corporateActionDiagnostics };
}
