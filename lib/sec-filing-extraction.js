...
const CAPITAL_EXPENDITURE_LABEL = /\b(?:capital expenditures?|capital additions|payments? to acquire property(?:,? plant(?:,? and)? equipment| and equipment)?|purchases? of property(?:,? plant(?:,? and)? equipment| and equipment)?|additions to property(?:,? plant(?:,? and)? equipment| and equipment)?)\b/i;
...
function financialScale(text) {
  const match = /\b(?:in|amounts? in|\$|USD|CAD|CNY|EUR|GBP|AUD|JPY|CHF)\s*(?:\(|:)?\s*(thousands?|millions?|billions?)\b/i.exec(String(text ?? ""));
  if (!match) return 1;
...
export function extractFilingCapitalExpenditureFacts({ html, form, filed, reportDate = null, accession, documentUrl, documentName }) {
  ...
  tables.forEach((tableMatch, tableIndex) => {
    ...
    const currency = financialCurrency(tableText); const scale = financialScale(tableText); const periods = filingPeriods(rows, form, reportDate);
