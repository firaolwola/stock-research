...
function effectiveDate(text, anchor = null) {
  ...
  const effectiveDateOf = [...textValue.matchAll(new RegExp(`\\b(?:effective date|date of effectiveness)\\b\\s*(?:(?:of|for)\\s+(?:the\\s+)?(?:action|split|which)\\s*)?(?:was|is|being)?\\s*(?::\\s*)?(${DATE_TOKEN_SOURCE})\\b`, "gi"))].map((match) => ({ match, actionAnchor: match.index }));
...
function splitDateRole(text, selectedDate, actionState, statedLifecycle, historicalList) {
  ...
  const rolePatterns = {
    ...
    effective_date: [
      ...
      new RegExp(`\\b(?:effective date|date of effectiveness)\\b\\s*(?:(?:of|for)\\s+(?:the\\s+)?(?:action|split|which)\\s*)?(?:was|is|being)?\\s*(?::\\s*)?(${DATE_TOKEN_SOURCE})\\b`, "gi")
    ],
    authorization_date: [
