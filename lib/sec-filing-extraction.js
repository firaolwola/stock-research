...
function controlWarningContext(text) {
  ...
  const match = negative.exec(text); if (!match) return null;
  const context = text.slice(Math.max(0, match.index - 220), Math.min(text.length, match.index + match[0].length + 800)).trim();
  if (/\b(?:no material weaknesses? (?:were|was|have been) identified|internal control over financial reporting was effective|maintained in all material respects|opinion.{0,120}effective internal control)\b/i.test(context)
    && !/\b(?:identified|unremediated|remained?|continues? to have)\s+(?:one or more\s+)?material weaknesses?\b|\b(?:was|were|is|are)\s+(?:not effective|ineffective)\b/i.test(context)) return null;
  return context;
}
...
export function extractSecFilingEvidence({ html, form, filed, accession, documentUrl, documentName, evaluatedAt }) {
  ...
  if (reverse && reverseRatio) {
    const action = resolvedSplitState(reverse, filed, evaluatedAt); const label = { completed: "Completed", scheduled: "Scheduled", authorized: "Authorized", proposed: "Proposed", cancelled: "Cancelled" }[action.state] ?? "Unresolved";
    add("reverse_split", "reverse_splits", `${label} ${reverseRatio.label} reverse split`, reverse, { ratio: reverseRatio.label, split_factor: reverseRatio.factor, action_state: action.state, effective_date: action.date });
  }
