...
function financialTrend(report, metricKey) {
  ...
  const periodShapeMatches = pointInTimeMetrics.has(metricKey)
    ...
    : durations.every((days) => Math.abs(days - durations[0]) <= 3) && (cadence !== "annual" || durations.every((days) => days >= 330 && days <= 400));
  ...
  const scored = result(report, scoreKey, "confirmed", value, `Company-relative trend only; raw company size is not scored. ${formula}`, components);
...
function splitRisk(report) {
  ...
  const pressure = c.items.some((x) => x.state === "confirmed" && x.kind === "exchange_compliance" && x.resolution_state !== "resolved");
